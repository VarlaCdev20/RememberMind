...
class AgendaEnfermeria extends Component
{
    ...
    public function recibirTurno(): void
    {
        ...
        \Illuminate\Support\Facades\DB::transaction(function () use ($turno) {
            $existente = RecepcionTurno::where('cod_turno', $turno->cod_turno)
                ->where('cod_usuario', Auth::id())->whereDate('fecha_hora_recepcion', today())
                ->lockForUpdate()->exists();
            if (! $existente) RecepcionTurno::create([
