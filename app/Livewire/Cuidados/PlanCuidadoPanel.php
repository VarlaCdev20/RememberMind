...
class PlanCuidadoPanel extends Component
{
    ...
    public function guardar(): void
    {
        ...
        if ($this->estadoPlan === 'ACTIVO') {
            $activoExistente = PlanCuidado::where('cod_residente', $this->codAm)
        ...
        if ($this->estadoPlan === 'ACTIVO') {
            $asignado = AsignacionResidenteJornada::where('cod_residente', $this->codAm)
    ...
    public function editarBorrador(string $id): void
    {
        ...
        $this->editandoId = $id;
        foreach (['codAm'=>'cod_residente','tipoPlan'=>'tipo_plan','nivelCuidado'=>'prioridad','estadoPlan'=>'estado','origen'=>'observacion','resumen'=>'objetivo_general'] as $prop => $col) $this->$prop = $plan->$col ?? '';
