...
class MedicacionAdultoModal extends Component
{
    ...
    public function abrirModalMedicacion($cod_am, $id_med = null): void
    {
        $this->autorizarGestionOrden($id_med ? 'prescripciones.editar' : 'prescripciones.crear');
        $this->resetValidation();
        $this->cod_am = (string) $cod_am;
    ...
    public function cargarDatos(): void
    {
        $prescripcion = Prescripcion::query()->with(['medicamento', 'horarios', 'personal'])
            ->where('cod_residente', $this->cod_am)->findOrFail($this->cod_med_adulto);
        ...
        $this->cod_am = $prescripcion->cod_residente;
    }
    ...
    public function guardar(): void
    {
        ...
        DB::transaction(function () use ($personal, $cantidad, $unidad): void {
            ...
            if ($this->isEditing) {
                $prescripcion = Prescripcion::query()->where('cod_residente', $this->cod_am)
                    ->findOrFail($this->cod_med_adulto);
            } else {
                $atencion = Atencion::query()->where('cod_residente', $this->cod_am)
                    ->where('cod_personal', $personal->cod_personal)
                    ->whereNotIn('estado', ['ANULADA', 'CANCELADA'])
                    ->latest('fecha_hora')->first();
                if (! $atencion) {
                    throw ValidationException::withMessages([
                        'cod_am' => 'Registre primero una atención médica; la prescripción V2 debe pertenecer a una atención.',
                    ]);
                }
    ...
    public function cambiarEstado($id, $estado): void
    {
        ...
        Prescripcion::query()->where('cod_residente', $this->cod_am)->findOrFail($id)
    ...
    public function render()
    {
        return view('livewire.medicacion.medicacion-adulto-modal', [
            ...
            'adultoSeleccionado' => $this->cod_am ? Residente::query()->find($this->cod_am) : null,
        ]);
    }
