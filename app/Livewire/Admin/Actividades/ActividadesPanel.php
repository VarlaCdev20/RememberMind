...
class ActividadesPanel extends Component
{
    ...
    public function actualizarActividad(): void
    {
        ...
        if ($participante) {
            $participante->update(['cod_residente' => $this->codAm]);
        } else {
            ParticipanteActividad::create(['cod_participante'=>$this->codigo('PAR'),'cod_actividad'=>$a->cod_actividad,'cod_residente'=>$this->codAm]);
        }
        $this->modalEditar = false;
        $this->resetForm();
