...
class DashboardMedico extends Component
{
    ...
    public function render()
    {
        ...
        $this->totalMedicacionActiva = Prescripcion::whereIn('estado', ['ACTIVO', 'ACTIVA'])->count();
        $this->notasHoy  = NotaEvolucionMedica::where('fecha', $hoy)->where('estado', 'ACTIVO')->count();
        $this->signosHoy = SignoVital::whereDate('fecha_hora', $hoy)->whereIn('estado', ['VIGENTE', 'ACTIVO'])->count();

        $this->alertasCriticas = SignoVital::whereIn('estado', ['VIGENTE', 'ACTIVO'])
            ->where(fn($q) => $q
        ...
        $this->alertasPacientes = SignoVital::whereIn('estado', ['VIGENTE', 'ACTIVO'])
            ->where(fn($q) => $q
                ->where('presion_sistolica', '>', 160)
                ->orWhere('presion_sistolica', '<', 90)
                ->orWhere('saturacion', '<', 92)
    ...
    private function renderV2()
    {
        ...
        $this->valoracionesPendientes = AdultoMayor::whereIn('estado', $estadosPendientes)
            ->orderBy('apellido_paterno')
            ->get();
        $this->totalResidentes = AdultoMayor::whereIn('estado', $estadosActivos)->count();
        $this->pendientesValoracion = $this->valoracionesPendientes->count();
        $this->enSeguimientoActivo = AdultoMayor::where('estado', 'EN_SEGUIMIENTO_ACTIVO')->count();
        $this->totalMedicacionActiva = Prescripcion::whereIn('estado', ['ACTIVA', 'ACTIVO'])->count();
        $this->notasHoy = NotaClinica::whereDate('fecha_hora', today())->count();
        $this->signosHoy = SignoVital::whereDate('fecha_hora', today())->count();
        $this->alertasCriticas = 0;
        $this->alertasPacientes = [];
        $this->notasRecientes = [];
        $this->chartEdad = ['labels' => [], 'masculino' => [], 'femenino' => []];
        $this->chartDiagnosticos = ['labels' => [], 'values' => []];
        $this->chartDependencia = ['labels' => [], 'values' => []];
        $this->chartImc = ['labels' => [], 'values' => []];
        $this->chartTendencia = ['labels' => [], 'pa' => [], 'fc' => [], 'sat' => []];
        $this->chartEstados = ['labels' => [], 'values' => []];
        $this->chartNotasTipo = ['labels' => [], 'values' => []];
    ...
    {
        $diagnosticos = \App\Models\Diagnostico::where('estado', '!=', 'ANULADO')
        ...
        if ($diagnosticos->isEmpty()) {
            return [
                'labels' => ['Hipertensión', 'Diabetes', 'Cardiopatía', 'ACV / Ictus', 'Alzheimer', 'Parkinson', 'Depresión', 'Dolor crónico'],
                'values' => [0, 0, 0, 0, 0, 0, 0, 0],
            ];
        }

        return [
            'labels' => $diagnosticos->pluck('nombre')->toArray(),
            'values' => $diagnosticos->pluck('total')->toArray(),
        ];
    }
    ...
    {
        $imcs = SignoVital::whereIn('estado', ['VIGENTE', 'ACTIVO'])
            ->whereNotNull('imc')
            ->where('imc', '>', 0)
            ->pluck('imc')
            ->map(fn($v) => (float)$v);
    ...
    {
        $datos = SignoVital::where('fecha_hora', '>=', now()->subDays(30))
            ->select(
