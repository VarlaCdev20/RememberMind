...
class UsuariosPanel extends Component
{
    ...
    // ── Campos del Usuario ──
    public ?string $cod_usuario = null;
    public $nombres;
    public $ap_paterno;
    public $ap_materno;
    ...
    private function guardarVinculosFamiliar(\App\Models\Contacto $contacto): void
    {
        foreach ($this->vinculosFamiliar as $vinculo) {
            ...
            $relacion = \App\Models\ResidenteContacto::firstOrNew([
                ...
                'cod_residente' => $vinculo['cod_residente'] ?? null,
            ]);
        ...
        return \App\Models\AdultoMayor::query()
            ...
            ->where(function ($q) use ($termino) {
                $q->whereLike('nombres', $termino)
                    ...
                    ->orWhereRaw("CONCAT_WS(' ', nombres, apellido_paterno, apellido_materno) ILIKE ?", [$termino]);
            })
            ...
            ->get()
            ->map(fn ($adultoMayor) => [
                'cod_residente' => $adultoMayor->cod_residente,
    ...
    public function updatedBusquedaAdultoMayor($value): void
    {
        ...
        if ($this->selected_cod_residente && $texto !== $this->adultoMayorSeleccionadoLabel) {
            $this->selected_cod_residente = '';
    ...
    public function seleccionarAdultoMayor($codResidente): void
    {
        ...
        $label = $this->obtenerEtiquetaAdultoMayor($adultoMayor);
        $this->selected_cod_residente = $adultoMayor->cod_residente;
        ...
        $this->resetErrorBag('selected_cod_residente');
    }
    ...
    public function rules()
    {
        $rules = [
            ...
            'correo' => [
                ...
                'email',
                'max:255',
                Rule::unique('usuarios', 'correo')->ignore($this->usuarioId, 'cod_usuario'),
                function ($attribute, $value, $fail) {
    ...
    private function validarUnicidadDocumento($fail)
    {
        ...
        if ($this->isEdit) {
            $query->where('cod_usuario', '!=', $this->usuarioId);
        }
        if ($query->exists()) {
    ...
    {
        $this->reset([
            'cod_usuario', 'nombres', 'ap_paterno', 'ap_materno', 'fecha_nacimiento', 'genero',
    ...
    public function vincularAdultoMayor()
    {
        ...
        $this->validate([
            'selected_cod_residente' => 'required',
        ...
        foreach ($this->vinculosFamiliar as $vinculo) {
            if (($vinculo['cod_residente'] ?? null) === $this->selected_cod_residente) {
                $this->dispatch('swal', [
                    'icon' => 'warning',
                    'title' => 'Ya vinculado',
        ...
        $this->vinculosFamiliar[] = [
            'cod_residente' => $this->selected_cod_residente,
    ...
    public function editarUsuario($cod_usuario)
    {
        ...
        if ($cod_usuario === 'USU_0001' && auth()->id() !== 'USU_0001') {
            $this->dispatch('swal', [
                'icon' => 'error',
                'title' => 'Acceso denegado',
        ...
        $usuario = User::with(['roles'])->findOrFail($cod_usuario);
    ...
    public function siguientePaso()
    {
        if ($this->pasoFormulario === 1) {
            ...
            if ($this->rol === 'FAMILIAR') {
                if (!empty($this->busquedaAdultoMayor) && empty($this->selected_cod_residente) && count($this->vinculosFamiliar) === 0) {
                    $this->addError('selected_cod_residente', 'Seleccione un adulto mayor válido de la lista.');
                    return;
                }

                if (!empty($this->selected_cod_residente) && !collect($this->vinculosFamiliar)->contains(fn ($v) => ($v['cod_residente'] ?? null) === $this->selected_cod_residente)) {
                    $this->vincularAdultoMayor();
                    if ($this->getErrorBag()->has('selected_cod_residente') || $this->getErrorBag()->has('selected_parentesco')) {
                        return;
                    }
                }
    ...
    private function guardarSubModelos(User $usuario)
    {
        if ($this->rol === 'FAMILIAR') {
            ...
            \App\Models\Personal::updateOrCreate(
                ['cod_usuario' => $usuario->cod_usuario],
                [
    ...
    private function procesarPostGuardado(User $usuario, ?string $passwordTemporal, ?string $oldPhotoToDelete)
    {
        ...
        if ($this->isEdit) {
            ...
            $passwordModificada = ($usuario->cod_usuario === auth()->id() && !empty($this->password));
            ...
            $this->documentosRequeridos = app(\App\Backend\Modulos\Documentos\Servicios\DocumentosUsuarioService::class)->documentosRequeridosPorRol($rolGuardado);
    ...
    public function abrirVistaCompleta($codUsuario)
    {
        $this->verUsuario($codUsuario);
    }
    ...
    public function abrirFichaRapida($codUsuario): void
    {
        $this->verUsuario($codUsuario);
    }
    ...
    public function verUsuario($codUsuario)
    {
        $this->usuarioDetalle = User::with([
            'roles',
            'documentos',
            'familiares.adultosMayores'
        ])->findOrFail($codUsuario);
    ...
    public function abrirEdicionDesdeDetalle()
    {
        if ($this->usuarioDetalle) {
            $this->editarUsuario($this->usuarioDetalle->cod_usuario);
        }
    }
    ...
    public function toggleEstado($id)
    {
        ...
        if ($usuario->cod_usuario === 'USU_0001' || $usuario->correo === 'admincasaamandita@gmail.com') {
            $this->dispatch('swal', [
                'icon' => 'error',
    ...
    public function restablecerPasswordUsuario($cod_usuario)
    {
        if (!auth()->user()->can('usuarios.editar')) {
            $this->dispatch('swal', [
                'icon' => 'error',
        ...
        $usuario = User::findOrFail($cod_usuario);

        // Protección absoluta de USU_0001
        if ($usuario->cod_usuario === 'USU_0001' && auth()->id() !== 'USU_0001') {
            $this->dispatch('swal', [
                'icon' => 'error',
                'title' => 'Acción no permitida',
        ...
        try {
            ...
            if (!empty($this->search)) {
                $query->where(function ($q) {
                    $q->whereLike('correo', '%' . $this->search . '%')
                      ->orWhereHas('personal', function ($pq) {
                          $pq->whereLike('nombres', '%' . $this->search . '%')
    ...
    public function enviarCorreoRequisitosAction($usuarioId = null)
    {
        try {
            ...
            $filename = 'solicitud_documental_' . $usuario->cod_usuario . '.pdf';
            ...
            Log::error("Error al enviar correo de requisitos para usuario " . ($usuario->cod_usuario ?? 'unknown') . ": " . $e->getMessage());
    ...
    public function reenviarCorreoRequisitos()
    {
        if ($this->usuarioPostRegistro) {
            $this->enviarCorreoRequisitosAction($this->usuarioPostRegistro->cod_usuario);
        }
    }
    ...
    {
        if ($this->usuarioPostRegistro) {
            $codUsuario = $this->usuarioPostRegistro->cod_usuario;
            $this->cerrarPostRegistro();
            $this->verUsuario($codUsuario);
        }
    }
    ...
    public function abrirModalSubirDoc($codTipoDoc)
    {
        ...
        $tipo = app(\App\Backend\Modulos\Documentos\Servicios\DocumentacionUsuarioService::class)->tipoDisponible($usuario, $codTipoDoc);
    ...
    public function guardarDocumento()
    {
        ...
        $service = app(\App\Backend\Modulos\Documentos\Servicios\DocumentacionUsuarioService::class);
        ...
        try {
            ...
            ])->findOrFail($usuario->cod_usuario);
    ...
    public function render()
    {
        ...
        if (!empty($this->search)) {
            $query->where(function ($q) {
                $q->whereLike('correo', '%' . $this->search . '%')
                  ->orWhereLike('cod_usuario', '%' . $this->search . '%')
                  ->orWhereHas('personal', function ($pq) {
                      $pq->whereLike('nombres', '%' . $this->search . '%')
                        ->orWhereLike('apellido_paterno', '%' . $this->search . '%')
                        ->orWhereLike('apellido_materno', '%' . $this->search . '%');
        ...
        $query->orderByRaw("CASE WHEN usuarios.estado = 'ACTIVO' THEN 0 ELSE 1 END")
              ->leftJoin('personal', 'usuarios.cod_usuario', '=', 'personal.cod_usuario')->select('usuarios.*')->orderBy('personal.nombres')->orderBy('personal.apellido_paterno')->orderBy('personal.apellido_materno');
        ...
        if ($this->mostrarFichaRapida && $this->usuarioFichaId) {
            $usuarioFichaModel = User::with([
                ...
                'documentos',
                'contactos.adultosMayores'
            ])->where('cod_usuario', $this->usuarioFichaId)->first();
        }
