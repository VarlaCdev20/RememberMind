...
class TurnosAsignacionesPanel extends Component
{
    ...
    */
    ...
    | salud            => asignaciones_personal vinculadas a jornadas V2
    | administrativo   => asignaciones_personal vinculadas a jornadas V2
    */
    ...
    */
    ...
    public function render(): View
    {
        ...
        $usuarioSeleccionado = $this->usuarioSeleccionado
            ? User::with(['roles'])->where('cod_usuario', $this->usuarioSeleccionado)->first()
            : null;

        return view('livewire.identidad.turnos-asignaciones-panel', [
            'areas' => $areas,
            'turnos' => $turnos,
            'roles' => $roles,
    ...
    */
    ...
    {
        return Area::query()->where('estado', 'ACTIVO')->orderBy('nombre')->get();
    }
    ...
    private function personalFiltrado(): Builder
    {
        ...
        if ($this->busqueda !== '') {
            ...
            $query->where(function (Builder $query) use ($busqueda) {
                $query->whereHas('personal', fn (Builder $personal) => $personal
                    ->whereLike('nombres', "%{$busqueda}%")
                    ->orWhereLike('apellido_paterno', "%{$busqueda}%")
                    ->orWhereLike('apellido_materno', "%{$busqueda}%"))
                    ->orWhereLike('correo', "%{$busqueda}%")
                    ->orWhereLike('cod_usuario', "%{$busqueda}%");
            });
        }
    ...
    */
    ...
    private function obtenerVirtualAsignaciones(): Collection
    {
        return AsignacionPersonal::query()
            ->with(['personal.usuario.roles', 'jornada.turno', 'area'])
            ->get()
            ->map(function (AsignacionPersonal $asignacion): object {
                $fecha = $asignacion->jornada?->fecha_jornada;
                $turno = $asignacion->jornada?->turno;

                return (object) [
                    'cod_asignacion' => $asignacion->cod_asignacion_personal,
                    'cod_usuario' => $asignacion->personal?->cod_usuario,
                    'usuario' => $asignacion->personal?->usuario,
                    'estado' => $asignacion->estado,
                    'dias_semana' => [$this->normalizarDiaSemana($fecha?->locale('es')->isoFormat('dddd'))],
                    'fecha_inicio' => $fecha?->copy()->startOfDay(),
                    'fecha_fin' => $fecha?->copy()->endOfDay(),
                    'cod_area' => $asignacion->cod_area,
                    'cod_turno' => $asignacion->jornada?->cod_turno,
                    'turno' => $turno ? (object) [
                        'nombre' => $turno->nombre,
                        'hora_inicio' => $turno->hora_inicio,
                        'hora_fin' => $turno->hora_cierre,
                    ] : null,
        ...
        if ($this->busqueda !== '') {
            ...
            $all = $all->filter(function ($asig) use ($busqueda) {
                return str_contains(mb_strtolower($asig->usuario?->nombres ?? ''), $busqueda)
                    ...
                    || str_contains(mb_strtolower($asig->usuario?->cod_usuario ?? ''), $busqueda);
            });
        }
        ...
        return User::with(['roles'])
            ...
            ->when($this->busquedaModal !== '', function (Builder $query) {
                ...
                $query->where(fn (Builder $query) => $query
                    ->whereLike('correo', "%{$busqueda}%")
                    ->orWhereLike('cod_usuario', "%{$busqueda}%")
                    ->orWhereHas('personal', fn (Builder $personal) => $personal
                        ->whereLike('nombres', "%{$busqueda}%")
                        ->orWhereLike('apellido_paterno', "%{$busqueda}%")
                        ->orWhereLike('apellido_materno', "%{$busqueda}%")));
            })
    ...
    */
    ...
    private function metricas(Collection $personal, Collection $asignaciones, Collection $turnos): array
    {
        ...
        $conHorario = $asignaciones->pluck('cod_usuario')->unique();
        ...
        return [
            'con_horario' => $personalActivo->whereIn('cod_usuario', $conHorario)->count(),
            'sin_horario' => $personalActivo->whereNotIn('cod_usuario', $conHorario)->count(),
            'en_turno_hoy' => $enTurno->pluck('cod_usuario')->unique()->count(),
            'disponibles_hoy' => max(0, $personalActivo->count() - $asignacionesHoy->pluck('cod_usuario')->unique()->count()),
    ...
    private function datosDia(Carbon $fecha, Collection $personal, Collection $asignaciones, array $conflictos): array
    {
        $filas = $personal->map(function (User $usuario) use ($fecha, $asignaciones, $conflictos) {
            $asignacionesUsuario = $asignaciones->where('cod_usuario', $usuario->cod_usuario);
    ...
    private function datosMes(Carbon $fecha, Collection $asignaciones, Collection $turnos, array $conflictos): array
    {
        ...
        for ($cursor = $inicio->copy(); $cursor <= $fin; $cursor->addDay()) {
            ...
            $dias->push([
                ...
                'asignados' => $eventos->pluck('cod_usuario')->unique()->count(),
    ...
    private function datosAnio(Carbon $fecha, Collection $asignaciones, Collection $turnos, array $conflictos): array
    {
        $meses = collect(range(1, 12))->map(function (int $mes) use ($fecha, $asignaciones, $turnos, $conflictos) {
            ...
            for ($dia = $inicio->copy(); $dia <= $fin; $dia->addDay()) {
                $eventos = $this->eventosSegunFiltro($this->eventosFecha($asignaciones, $dia), $dia, $conflictos);
                $asignados = $asignados->merge($eventos->pluck('cod_usuario'));
    ...
    */
    ...
    private function detectarConflictos(Collection $asignaciones): array
    {
        ...
        foreach ($asignaciones->groupBy('cod_usuario') as $items) {
            $items = $items->values();
    ...
    */
    ...
    public function guardarAsignacionPlaza(): void
    {
        ...
        $codUsuario = $this->enfermeroSeleccionado ?: null;
