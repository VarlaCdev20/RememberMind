...
class PacientesSeguimientoPanel extends Component
{
    ...
    public function render()
    {
        ...
        // Last vitals and notes per patient (keyed by cod_residente)
        $codResidentes = $pacientes->pluck('cod_residente')->toArray();
        ...
        $ultimasNotas = NotaClinica::whereIn('cod_residente', $codResidentes)
            ->where('estado', 'ACTIVO')
            ->orderByDesc('fecha_hora')
            ->get()
            ->keyBy('cod_residente');

        $cntMedicacion = Prescripcion::whereIn('cod_residente', $codResidentes)
            ->where('estado', 'ACTIVA')
