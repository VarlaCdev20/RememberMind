...
class PaseTurnoPanel extends Component
{
    ...
    public function abrirGenerar(?string $codResidente = null): void
    {
        if ($codResidente) {
            $this->abrirPrepararPase($codResidente);
        }
    ...
    public function generarPase(): void
    {
        ...
        $cod = $this->codResidente ?: $this->codResidenteSeleccionado;
        if (!$cod) {
            $this->addError('codResidente', 'Seleccione un residente.');
            return;
        }
