...
class DeteccionAlertasService
{
    ...
    {
        $creadas = 0;
        SignoVital::vigentes()->when($codAm, fn ($q) => $q->where('cod_residente', $codAm))->orderBy('cod_signo')->chunk(100, function ($registros) use (&$creadas) {
        ...
        AdministracionMedicacion::query()->when($codAm, fn ($q) => $q->where('cod_residente', $codAm))->where('resultado', 'OMITIDA')->orderBy('cod_administracion')->chunk(100, function ($registros) use (&$creadas) {
        ...
        EjecucionCuidado::whereIn('estado', ['PENDIENTE', 'EN_PROCESO', 'OMITIDA'])
            ->when($codAm, fn ($q) => $q->where('cod_residente', $codAm))
        ...
        Atencion::query()->when($codAm, fn ($q) => $q->where('cod_residente', $codAm))
    ...
    public function registrarPreventivaSiNoExiste(
        ...
        return DB::transaction(function () use ($codAm, $origen, $tipoAlerta, $nivel, $motivo) {
            $existe = Alerta::where('cod_residente', $codAm)
                ->where('modulo', $origen)
    ...
    private function registrar($registro, string $origen, string $tipo, string $nivel, string $texto): int
    {
        ...
        return DB::transaction(function () use ($registro, $origen, $tipo, $nivel, $motivo, $texto) {
            ...
            if (Alerta::where('cod_residente', $registro->cod_residente ?? $registro->cod_am)->where('modulo', $origen)
                ->where('descripcion', 'like', $referencia.'%')->exists()) {
                return 0;
            }

            // 2. Idempotencia por tipo y condición activa (ABIERTA o EN_ATENCION)
            $alertaEquivalente = Alerta::where('cod_residente', $registro->cod_residente ?? $registro->cod_am)
                ->where('modulo', $origen)
                ->where('tipo', $tipo)
                ->whereIn('estado', ['ABIERTA', 'EN_ATENCION'])
            ...
            Alerta::create([
                'cod_alerta' => 'ALA_' . strtoupper(\Illuminate\Support\Str::random(10)),
                'cod_residente' => $registro->cod_residente ?? $registro->cod_am,
                'tipo' => $tipo,
                'prioridad' => $nivel,
                'modulo' => $origen,
