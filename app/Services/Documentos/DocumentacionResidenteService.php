...
class DocumentacionResidenteService
{
     ...
     */
    public function obtenerDocumentosResidente(AdultoMayor $adulto): Collection
    {
        // 1. Obtener registros persistidos en la base de datos
        $documentosBd = DocumentoAdultoMayor::query()
    ...
    public function obtenerDocumentos(AdultoMayor $adulto): Collection
    {
     ...
     */
    public function normalizarDocumentoBd(DocumentoAdultoMayor $doc, AdultoMayor $adulto): array
    {
        ...
        $urlArchivo = route('admin.adultos-mayores.documentos.archivo', [
            'adulto_mayor' => $adulto->cod_am,
            'documento' => $doc->cod_doc_am,
        ]);

        $urlPreview = route('admin.adultos-mayores.documentos.archivo', [
            'adulto_mayor' => $adulto->cod_am,
            'documento' => $doc->cod_doc_am,
            'preview' => 1,
        ]);

        $fechaStr = $doc->fecha_subida ? (is_string($doc->fecha_subida) ? $doc->fecha_subida : $doc->fecha_subida->format('Y-m-d')) : now()->format('Y-m-d');
        $fechaFormateada = date('d/m/Y', strtotime($fechaStr));
        $horaFormateada = $doc->created_at ? $doc->created_at->format('H:i') : '09:30';

        $subidoPor = 'Personal Asistencial';
        $subidoPorArea = 'Servicio Asistencial';

        return [
            'id' => 'doc_bd_' . $doc->cod_doc_am,
            'bd_id' => $doc->cod_doc_am,
            'nombre' => $doc->nombre ?? 'Documento sin título',
            'descripcion' => $doc->observaciones ?? 'Documento registrado en el expediente del residente.',
            'categoria' => $categoria,
            ...
            'relacionado_con' => $doc->modulo_ref ?? 'Expediente clínico general',
            'observaciones' => $doc->observaciones ?? 'Documento validado y archivado de acuerdo a protocolo institucional.',
            'trazabilidad' => [
                [
     ...
     */
    public function obtenerDocumentosBase(AdultoMayor $adulto): array
    {
        return [
            [
                'id' => 'doc_ref_01',
