...
class ResultadosEstudiosService
{
     ...
     */
    public function obtenerEstudios(AdultoMayor $adultoMayor)
    {
        $items = collect();

        // 1. Integrar documentos reales que correspondan a estudios médicos / analíticas
        $docsReales = DocumentoAdultoMayor::where('cod_am', $adultoMayor->cod_am)
            ->where(function($q) {
                $q->whereIn('tipo_documento', ['MEDICO', 'ESTUDIO', 'LABORATORIO', 'EXAMEN', 'IMAGEN'])
                  ->orWhere('nombre', 'like', '%laboratorio%')
                  ->orWhere('nombre', 'like', '%certificado%')
                  ->orWhere('nombre', 'like', '%estudio%')
                  ->orWhere('nombre', 'like', '%informe%');
            })
            ->get();

        foreach ($docsReales as $doc) {
            $esCertificado = str_contains(strtolower($doc->nombre), 'certificado');
            $tipoCat = $esCertificado ? 'OTROS' : 'LABORATORIO';
            $fecha = $doc->fecha_subida ? Carbon::parse($doc->fecha_subida) : Carbon::parse($doc->created_at);

            $items->push([
                'id' => 'DOC_' . $doc->cod_doc_am,
                'cod_doc' => $doc->cod_doc_am,
                ...
                'observaciones' => $doc->observaciones ?: 'Documentación médica evaluada al ingreso del residente. Constancia archivada.',
                ...
                'nombre_documento' => $doc->nombre . '.pdf',
