...
class AdultoMayorBitacoraService
{
     ...
     *
     * @param  string  $codResidente       Código del adulto mayor (ej. AM_0001)
    ...
    public function obtenerBitacora(
        ...
        $query = DB::table('activity_log')
            ...
            ->where(function ($q) use ($codResidente) {
                ...
                ->orWhere(function ($q2) use ($codResidente) {
     ...
     */
    public function obtenerTiposEventos(string $codResidente): Collection
    {
        if (!Schema::hasTable('activity_log')) {
            return collect();
        }
        ...
        return DB::table('activity_log')
            ->where(function ($q) use ($codResidente) {
                $q->where('subject_id', $codResidente)
                  ->orWhere('description', 'like', '%' . $codResidente . '%');
            })
            ->whereNotNull('event')
            ->distinct()
            ->pluck('event')
