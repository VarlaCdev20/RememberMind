...
class GeneradorPlanillaEnfermeriaService
{
    ...
    protected function generarDia(
        ...
        foreach ($patronDia as $codigoTurno => $codigosBase) {
            ...
            foreach ($codigosBase as $posicionEnTurno => $codigoBase) {
                ...
                $titular_cod = ($titularBase && !empty($titularBase['cod_usuario'])) ? $titularBase['cod_usuario'] : null;
                $titular_nom = ($titularBase && !empty($titularBase['cod_usuario'])) ? $titularBase['nombre'] : 'Sin enfermero asignado';
                ...
                if (isset($this->temporales[$tempKey]) && !empty($this->temporales[$tempKey])) {
                    ...
                    $reemplazo_cod = $user ? ($user['cod_usuario'] ?? null) : null;
                ...
                $asignaciones[] = [
                    'codigo' => $codigoRotado,
                    'cod_usuario' => $active_enfermero['cod_usuario'] ?? null,
    ...
    protected function aplicarFiltros(array $resultado, array $config): array
    {
        ...
        if ($config['solo_conflictos']) {
            $resultado['alertas'] = collect($resultado['alertas'])
                ->whereIn('prioridad', ['CRITICO', 'ALTO', 'CRITICA', 'ALTA'])
                ->values()
                ->all();
        }
    ...
    public function obtenerTurnoEnFecha(string $codUsuario, $fecha): ?array
    {
        ...
        foreach ($semana['dias'] as $dia) {
            if (($dia['fecha'] ?? null) === $fechaStr) {
                foreach ($dia['turnos'] as $turno) {
                    foreach ($turno['asignaciones'] ?? [] as $asignacion) {
                        if (($asignacion['cod_usuario'] ?? null) === $codUsuario) {
                            return $asignacion;
                        }
                    }
                }
