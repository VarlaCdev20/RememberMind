...
class BitacoraController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = DB::table('activity_log')
            ->leftJoin('usuarios', 'activity_log.causer_id', '=', 'usuarios.cod_usuario')
            ...
            ->select([
                ...
                'personal.nombres as causer_nombres',
                'personal.apellido_paterno as causer_ap_paterno',
                'usuarios.correo as causer_correo',
            ])
        ...
        if ($request->filled('buscar')) {
            ...
            $query->where(function($q) use ($buscar) {
                $q->where('activity_log.description', 'like', "%$buscar%")
                  ...
                  ->orWhere('personal.nombres', 'like', "%$buscar%")
                  ->orWhere('personal.apellido_paterno', 'like', "%$buscar%");
            });
        }
    ...
    private function obtenerInfoModulo(string $name): array
    {
        ...
        if (str_contains($name, 'app\\models\\')) {
            $name = match($name) {
                ...
                'app\\models\\contacto', 'app\\models\\residentecontacto' => 'familiares',
                'app\\models\\notaclinica' => 'observaciones',
                'app\\models\\atencion' => 'atenciones',
                'app\\models\\actividad', 'app\\models\\participanteactividad' => 'actividades',
                'app\\models\\aplicacioninstrumento' => 'cognitivo',
                'app\\models\\documento' => 'documentos',
                'app\\models\\prescripcion' => 'medicación',
                'app\\models\\administracionmedicacion' => 'medicación',
                'app\\models\\signovital' => 'signos vitales',
                'app\\models\\valoracionfuncional' => 'valoración funcional',
                'app\\models\\historialestadoresidente' => 'historial',
    ...
    private function identificarRegistroAfectado($log): string
    {
        ...
        try {
            return match ($log->subject_type) {
                'App\\Models\\AdultoMayor', 'App\\Models\\Residente' => DB::table('residentes')->where('cod_residente', $log->subject_id)->value(DB::raw("TRIM(nombres || ' ' || apellido_paterno)")) ?? "Residente: $log->subject_id",
                'App\\Models\\User' => DB::table('personal')->where('cod_usuario', $log->subject_id)->value(DB::raw("TRIM(nombres || ' ' || apellido_paterno)")) ?? "Usuario: $log->subject_id",
                'App\\Models\\Contacto' => DB::table('contactos')->where('cod_contacto', $log->subject_id)->value(DB::raw("TRIM(nombres || ' ' || apellido_paterno)")) ?? "Contacto: $log->subject_id",
                'App\\Models\\Documento' => "Doc: " . (DB::table('documentos')->where('cod_documento', $log->subject_id)->value('nombre') ?? $log->subject_id),
                'App\\Models\\Prescripcion' => "Prescripción: " . (DB::table('prescripciones')->where('cod_prescripcion', $log->subject_id)->value('indicacion') ?? $log->subject_id),
