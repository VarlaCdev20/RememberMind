...
use App\Http\Requests\Admin\StoreAdultoMayorRequest;
use App\Http\Requests\Admin\UpdateAdultoMayorRequest;
use App\Services\Admin\AdultoMayorService;
use App\Services\Admin\AdultoMayorBitacoraService;
...
class AdultoMayorController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.adultos-mayores.index', compact(
            'adultos', 
            'totales', 
            'estadosAdulto', 
            'tiposActividad', 
    ...
    public function create()
    {
        ...
        return view('admin.adultos-mayores.create', compact('estadosAdulto'));
    }
...
public function show(AdultoMayor $adulto_mayor)
{
    ...
    return view('admin.adultos-mayores.show', compact(
        'adulto',
        'familiaresActivos',
        'familiaresInactivos',
    ...
    public function reporteIndividual(Request $request, AdultoMayor $adulto_mayor)
    {
        ...
        $bitacora = \Spatie\Activitylog\Models\Activity::where('subject_type', AdultoMayor::class)
        ...
        if ($format === 'pdf') {
            $pdf = Pdf::loadView('admin.adultos-mayores.reportes.pdf_individual', $viewData)->setPaper('a4', 'portrait');
    ...
    public function reporteEspecifico(Request $request, AdultoMayor $adulto_mayor, $tipo)
    {
        ...
        $applyDateFilter = function ($query, $dateColumn) use ($startDate, $endDate) {
            if ($startDate) {
                $query->where($dateColumn, '>=', \Carbon\Carbon::parse($startDate)->startOfDay());
            }
            if ($endDate) {
                $query->where($dateColumn, '<=', \Carbon\Carbon::parse($endDate)->endOfDay());
            }
            return $query;
        };
        ...
        if ($format === 'pdf') {
            $pdf = Pdf::loadView('admin.adultos-mayores.reportes.pdf_especifico', $viewData)->setPaper('a4', 'portrait');
            return $pdf->download("{$titulo}_{$adulto->cod_am}.pdf");
        }

        return view('admin.adultos-mayores.reportes.pdf_especifico', $viewData);
    }
    ...
    public function reporteInstitucional()
    {
        ...
        return view('admin.adultos-mayores.reportes.institucional', compact('stats'));
    }
    ...
    public function edit(AdultoMayor $adulto_mayor)
    {
        ...
        return view('admin.adultos-mayores.edit', compact('adulto', 'estadosAdulto'));
    }
    ...
    public function cambiarEstado(\App\Http\Requests\Admin\AdultosMayores\Salud\StoreCambioEstadoRequest $request, AdultoMayor $adulto_mayor)
    {
        try {
            ...
            if ($esArchivoOInactivo) {
                $adulto_mayor->archivado_en = \Carbon\Carbon::now();
            ...
            $historial = \App\Models\HistorialEstadoAdulto::create([
