<x-sistema-layout>
 ...
 <a href="{{ route('admin.bitacora.index') }}" class="inline-flex h-9 items-center justify-center gap-2 rounded-xl bg-fondo-app px-5 text-xs font-bold text-titulo transition hover:bg-fondo-panel active:scale-95">
 <i class="ph-bold ph-arrow-counter-clockwise"></i> Limpiar
 </a>
 <button type="submit" class="inline-flex h-9 items-center justify-center gap-2 rounded-xl bg-boton-principal px-6 text-xs font-bold text-inverso shadow-lg shadow-azul-profundo/20 transition hover:-translate-y-0.5 active:scale-95">
 ...
 <span class="rounded-full bg-boton-acento px-4 py-1 text-[10px] font-bold uppercase tracking-widest text-inverso shadow-sm">
 {{ $log->fecha_dia }}
 </span>
 <div class="h-px flex-1 bg-fondo-panel"></div>
 </div>
 ...
 <span class="text-xs font-bold text-titulo">{{ $log->evento_info['nombre'] }}</span>
 <span class="px-2 py-0.5 rounded-lg border {{ $log->modulo_info['color'] }} text-[9px] font-bold uppercase tracking-tighter flex items-center gap-1">
 ...
 <div class="flex items-center gap-2">
 <span class="text-[9px] font-bold uppercase tracking-widest text-titulo/30">Registro afectado:</span>
 ...
 <p class="text-lg font-extrabold text-titulo/40">No hay actividad registrada</p>
 ...
 <th class="px-5 py-4 text-[10px] font-bold uppercase tracking-widest text-titulo/60">Usuario y Rol</th>
 <th class="px-5 py-4 text-[10px] font-bold uppercase tracking-widest text-titulo/60">Módulo</th>
 <th class="px-5 py-4 text-[10px] font-bold uppercase tracking-widest text-titulo/60">Acción</th>
 <th class="px-5 py-4 text-[10px] font-bold uppercase tracking-widest text-titulo/60">Registro Afectado</th>
 </tr>
 </thead>
 ...
 <span class="text-xs font-bold text-titulo">{{ $log->fecha_formateada }}</span>
 <span class="text-[10px] font-medium text-titulo/40">{{ $log->fecha_relativa }}</span>
 </div>
 </td>
 <td class="px-5 py-4">
 <div class="flex items-center gap-3">
 <div class="h-8 w-8 rounded-xl bg-fondo-panel flex items-center justify-center text-titulo font-bold text-[10px]">
 {{ substr($log->causer_nombre, 0, 1) }}
 </div>
 <div>
 ...
 <td class="px-5 py-4">
 <span class="inline-flex items-center gap-1.5 rounded-lg border {{ $log->modulo_info['color'] }} px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest">
 <i class="ph-bold {{ $log->modulo_info['icono'] }}"></i>
 {{ $log->modulo_info['nombre'] }}
 </span>
 </td>
 ...
 <span class="text-[10px] font-bold uppercase tracking-tighter text-titulo">{{ $log->evento_info['nombre'] }}</span>
 </div>
 ...
 <p class="text-lg font-extrabold text-titulo/40">Sin resultados</p>
 </td>
 </tr>
 @endforelse
 </tbody>
