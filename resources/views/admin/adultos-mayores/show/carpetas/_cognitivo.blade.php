<div class="rounded-[24px] border border-borde bg-fondo-card/95 p-6 shadow-sm">
    <div class="mb-5 flex items-center justify-between border-b border-borde pb-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-lg font-black text-titulo">Cognitivo Resumido</h2>
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
                <div class="rounded-xl border border-borde-suave bg-fondo-panel p-3">
                    <span class="block text-xs font-black uppercase tracking-wide text-apoyo mb-1">Pruebas Totales</span>
                    <p class="text-xl font-black text-parrafo">{{ $evaluacionesLista->count() }}</p>
                </div>
                <div class="rounded-xl border border-borde-suave bg-fondo-panel p-3">
                    <span class="block text-xs font-black uppercase tracking-wide text-apoyo mb-1">Deterioro Detectado</span>
                    <p class="text-xl font-black {{ $evaluacionesLista->whereIn('nivel_riesgo', ['MEDIO', 'ALTO', 'MODERADO'])->count() > 0 ? 'text-amber-600' : 'text-parrafo' }}">
                        {{ $evaluacionesLista->whereIn('nivel_riesgo', ['MEDIO', 'ALTO', 'MODERADO'])->count() }}
                    </p>
                </div>
                <div class="rounded-xl border border-borde-suave bg-fondo-panel p-3">
                    <span class="block text-xs font-black uppercase tracking-wide text-apoyo mb-1">Evolución Promedio</span>
                    <p class="text-xl font-black text-titulo">{{ number_format($evaluacionesLista->avg('puntaje_total'), 1) }}</p>
                </div>
                <div class="rounded-xl border border-borde-suave bg-fondo-panel p-3">
                    <span class="block text-xs font-black uppercase tracking-wide text-apoyo mb-1">Tendencia</span>
                    <p class="text-sm font-black text-titulo mt-1.5">
            ...
            <div>
                <span class="block text-xs font-black uppercase tracking-wide text-apoyo mb-3">Última Prueba Aplicada</span>
                <div class="grid sm:grid-cols-3 gap-4 rounded-xl border border-borde-suave bg-fondo-card p-5">
                    <div>
                        <span class="block text-xs font-black uppercase tracking-wide text-apoyo mb-1">Tipo de Prueba</span>
                        <p class="text-base font-black text-parrafo">{{ optional($ultima->tipoEvaluacion)->nombre ?? 'Evaluación' }}</p>
                        <p class="text-xs font-bold text-apoyo">{{ \Carbon\Carbon::parse($ultima->fecha_eval)->format('d/m/Y') }}</p>
                    </div>
                    <div>
                        <span class="block text-xs font-black uppercase tracking-wide text-apoyo mb-1">Resultado Obtenido</span>
                        <p class="text-xl font-black text-titulo">{{ $ultima->puntaje_total }} <span class="text-xs font-bold text-apoyo">/ {{ optional($ultima->tipoEvaluacion)->puntaje_maximo ?? 30 }}</span></p>
                    </div>
                    <div>
                        <span class="block text-xs font-black uppercase tracking-wide text-apoyo mb-1">Interpretación Clínica</span>
                        <p class="text-xs font-bold {{ $colorRiesgo }}">{{ $ultima->resultado_interpretacion ?? 'N/D' }}</p>
                        <span class="inline-block mt-1 px-1.5 py-0.5 rounded text-xs font-black uppercase tracking-wide bg-fondo-panel text-titulo">Riesgo: {{ $riesgo }}</span>
                    </div>
                </div>
            </div>
