<div class="rounded-[24px] border border-borde bg-fondo-card/95 p-6 shadow-sm">
    <div class="mb-5 flex items-center justify-between border-b border-borde pb-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-lg font-black text-titulo">Salud y Cuidados Resumida</h2>
    ...
    @if($fichasMedicas->isNotEmpty() || $signosVitales->count() > 0 || $medicaciones->count() > 0 || $valoracionesFuncionales->count() > 0)
        <div class="space-y-6">
            ...
            {{-- Ficha Médica --}}
            <div>
                <span class="block text-xs font-black uppercase tracking-wide text-apoyo mb-3">Ficha Médica</span>
                @if($fichasMedicas->isNotEmpty())
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div class="rounded-xl bg-fondo-panel p-3 border border-borde">
                            <span class="block text-xs font-black uppercase tracking-wide text-apoyo mb-1">Última Act.</span>
                            <p class="text-xs font-black text-titulo">{{ $ficha->updated_at->format('d/m/Y') }}</p>
                        </div>
                        <div class="rounded-xl bg-fondo-panel p-3 border border-borde">
                            <span class="block text-xs font-black uppercase tracking-wide text-red-600/70 mb-1">Alergias</span>
                            <p class="text-xs font-black text-titulo truncate">{{ $ficha->alergias ?: 'Ninguna' }}</p>
                        </div>
                        <div class="rounded-xl bg-fondo-panel p-3 border border-borde">
                            <span class="block text-xs font-black uppercase tracking-wide text-apoyo mb-1">Enfermedades</span>
                            <p class="text-xs font-black text-titulo truncate">{{ $ficha->enfermedades_preexistentes ?: 'Ninguna' }}</p>
                        </div>
                    </div>
                @else
            ...
            {{-- Signos Vitales --}}
            <div>
                <span class="block text-xs font-black uppercase tracking-wide text-apoyo mb-3">Último Control de Signos Vitales</span>
                @if($signosVitales->count() > 0)
                    ...
                    <div class="flex flex-wrap items-center gap-4 rounded-xl border border-borde-suave bg-fondo-panel p-4">
                        <div>
                            ...
                            <p class="text-sm font-black text-titulo">{{ \Carbon\Carbon::parse($ultimoSigno->fecha)->format('d/m/Y') }}</p>
                        </div>
                        <div>
                            <span class="block text-xs font-bold text-apoyo uppercase">PA</span>
                            <p class="text-sm font-black text-titulo">{{ $ultimoSigno->presion_arterial ?? '--' }}</p>
                        </div>
                        <div>
                            <span class="block text-xs font-bold text-apoyo uppercase">FC</span>
                            <p class="text-sm font-black text-titulo">{{ $ultimoSigno->frecuencia_cardiaca ?? '--' }} <span class="text-xs font-normal">bpm</span></p>
                        </div>
                        <div>
                            <span class="block text-xs font-bold text-apoyo uppercase">FR</span>
                            <p class="text-sm font-black text-titulo">{{ $ultimoSigno->frecuencia_respiratoria ?? '--' }} <span class="text-xs font-normal">rpm</span></p>
                        </div>
                        <div>
                            <span class="block text-xs font-bold text-apoyo uppercase">Temp</span>
                            <p class="text-sm font-black text-titulo">{{ $ultimoSigno->temperatura ?? '--' }} <span class="text-xs font-normal">°C</span></p>
                        </div>
                        <div>
                            <span class="block text-xs font-bold text-apoyo uppercase">SpO2</span>
                            <p class="text-sm font-black text-titulo">{{ $ultimoSigno->saturacion_oxigeno ?? '--' }} <span class="text-xs font-normal">%</span></p>
                        </div>
                    </div>
                @else
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    <span class="block text-xs font-black uppercase tracking-wide text-apoyo mb-3">Medicación Activa</span>
                ...
                <div>
                    <span class="block text-xs font-black uppercase tracking-wide text-apoyo mb-3">Valoración Funcional (Barthel)</span>
