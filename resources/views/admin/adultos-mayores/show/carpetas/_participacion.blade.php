<div class="rounded-[24px] border border-borde bg-fondo-card/95 p-6 shadow-sm">
    <div class="mb-5 flex items-center justify-between border-b border-borde pb-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-lg font-black text-titulo">Participación Institucional</h2>
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 text-center">
                <div class="rounded-xl border border-borde-suave bg-fondo-panel p-3">
                    <span class="block text-xs font-black uppercase tracking-wide text-apoyo mb-1">Actividades Asignadas</span>
                    <p class="text-xl font-black text-parrafo">{{ $actividadesLista->count() }}</p>
                </div>
                <div class="rounded-xl border border-borde-suave bg-fondo-panel p-3">
                    <span class="block text-xs font-black uppercase tracking-wide text-apoyo mb-1">Actividades Asistidas</span>
                    <p class="text-xl font-black text-parrafo">
                ...
                <div class="rounded-xl border border-borde-suave bg-fondo-panel p-3 sm:col-span-1 col-span-2">
                    <span class="block text-xs font-black uppercase tracking-wide text-apoyo mb-1">Última Participación</span>
                    <p class="text-xs font-black text-titulo mt-2">
            ...
            <div>
                <span class="block text-xs font-black uppercase tracking-wide text-apoyo mb-3">Participaciones Recientes</span>
                <div class="space-y-3">
                    @foreach($actividadesLista->take(5) as $act)
                        <div class="flex items-center gap-4 rounded-xl border border-borde-suave bg-fondo-card p-3">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-black text-titulo truncate">{{ optional($act->tipoActividad)->tipo ?? 'Actividad' }}</p>
                                <p class="text-xs font-bold text-apoyo mt-0.5">{{ \Carbon\Carbon::parse($act->fecha)->format('d/m/Y') }} {{ $act->hora ? ' • '.substr($act->hora, 0, 5) : '' }}</p>
                            </div>
                            <span class="shrink-0 inline-block px-2 py-0.5 rounded text-xs font-black uppercase tracking-wide {{ in_array($act->estado, ['COMPLETADA', 'REALIZADA']) ? 'bg-fondo-panel text-parrafo' : 'bg-amber-100 text-amber-700' }}">
