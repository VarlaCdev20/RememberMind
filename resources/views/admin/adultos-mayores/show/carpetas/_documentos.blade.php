<div class="rounded-[24px] border border-borde bg-fondo-card/95 p-6 shadow-sm">
 ...
 <div>
 <h2 class="text-lg font-extrabold text-titulo">Expediente Documental</h2>
 ...
 <span class="block text-xs font-bold uppercase tracking-wide text-apoyo mb-1">Total Documentos</span>
 <p class="text-2xl font-black text-titulo">{{ $documentosLista->count() }}</p>
 </div>
 <div class="rounded-xl bg-fondo-panel p-3 border border-borde">
 <span class="block text-xs font-bold uppercase tracking-wide text-apoyo mb-1">Último Ingresado</span>
 <p class="text-sm font-bold text-titulo mt-2">{{ $documentosLista->first()->created_at->format('d/m/Y') }}</p>
 </div>
 </div>

 <div>
 <span class="block text-xs font-bold uppercase tracking-wide text-apoyo mb-3">Listado Resumido</span>
 ...
 <div class="min-w-0">
 <p class="truncate text-xs font-bold text-titulo">{{ $doc->titulo ?? $doc->nombre_original }}</p>
 ...
 <a href="{{ route('admin.adultos-mayores.documentos.index', $idAdulto) }}" class="inline-flex items-center gap-2 rounded-xl bg-boton-principal px-5 py-2.5 text-xs font-bold text-inverso shadow-sm hover:bg-fondo-panel transition">
