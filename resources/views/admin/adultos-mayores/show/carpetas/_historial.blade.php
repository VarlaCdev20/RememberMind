<div class="rounded-[24px] border border-borde bg-fondo-card/95 p-6 shadow-sm">
 ...
 <div>
 <h2 class="text-lg font-extrabold text-titulo">Historial Institucional</h2>
 ...
 <div>
 <span class="block text-xs font-bold uppercase tracking-wide text-apoyo mb-3">Últimas Notas Relevantes</span>
 @if($observacionesLista->count() > 0)
 <div class="space-y-4">
 @foreach($observacionesLista->take(4) as $obs)
 <div class="border-l-2 border-borde pl-3 py-1">
 <p class="text-xs font-bold uppercase tracking-wide text-apoyo mb-1">{{ \Carbon\Carbon::parse($obs->fecha)->format('d/m/Y') }} • {{ $obs->tipo_obs ?? 'Nota' }}</p>
 ...
 <div>
 <span class="block text-xs font-bold uppercase tracking-wide text-apoyo mb-3">Últimos Movimientos</span>
 ...
 <p class="text-xs font-bold text-titulo mt-0.5">{{ $mov['etiqueta'] ?? 'Actualización' }}</p>
