...
>
    ...
    <div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-borde pb-5">
        ...
        <div class="flex items-center gap-2">
            <a href="#" class="inline-flex items-center justify-center gap-2 rounded-xl bg-fondo-card border border-borde px-4 py-2.5 text-xs font-black text-titulo shadow-sm transition hover:-translate-y-0.5 hover:bg-fondo-panel active:scale-[0.98]">
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <div class="rounded-[24px] border border-borde bg-fondo-card/95 p-6 shadow-sm flex flex-col justify-center text-center">
            <p class="text-[10px] font-black uppercase tracking-[0.18em] text-apoyo">Estado Actual</p>
            <p class="mt-2 text-xl font-black {{ $estadoTexto === 'ACTIVO' ? 'text-emerald-600' : 'text-parrafo' }}">
        ...
        <div class="rounded-[24px] border border-borde bg-fondo-card/95 p-6 shadow-sm flex flex-col justify-center text-center">
            <p class="text-[10px] font-black uppercase tracking-[0.18em] text-apoyo">Movimientos Registrados</p>
        ...
        <div class="rounded-[24px] border border-borde bg-fondo-card/95 p-6 shadow-sm flex flex-col justify-center text-center">
            <p class="text-[10px] font-black uppercase tracking-[0.18em] text-apoyo">Última Modificación</p>
    ...
    <div class="rounded-[24px] border border-borde bg-fondo-card/95 p-6 shadow-sm">
        <h3 class="text-lg font-black text-titulo border-b border-borde pb-4 mb-4">Últimos Registros Relevantes</h3>
        ...
        @if(collect($bitacoraLista)->count() > 0)
                    ...
                    <div class="relative pl-6">
                        ...
                        <div class="rounded-xl border border-borde bg-fondo-panel p-4 transition hover:bg-fondo-panel">
                            <div class="flex items-center justify-between gap-4 mb-2">
                                <h4 class="text-sm font-black text-titulo"><i class="{{ $log['icono'] }} {{ $log['color_text'] }} mr-1"></i> {{ $log['etiqueta'] }}</h4>
                                <span class="text-[10px] font-bold text-apoyo">{{ $log['fecha_relativa'] }}</span>
                            </div>
                            
                            @if($log['modulo'])
                                <span class="inline-block px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest bg-fondo-panel text-parrafo mb-2">
                                    {{ $log['modulo'] }}
                                </span>
                            @endif
            ...
            <div class="mt-4 text-center">
                <a href="#" class="inline-flex items-center justify-center gap-2 rounded-xl bg-fondo-card border border-borde px-6 py-2.5 text-xs font-black uppercase tracking-wider text-parrafo shadow-sm transition hover:bg-fondo-panel active:scale-[0.98]">
