...
>
    ...
    <div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-borde pb-5">
        ...
        <div class="flex items-center gap-2">
            <button type="button" @click="abrir('documento')" class="inline-flex items-center justify-center gap-2 rounded-xl bg-fondo-card border border-borde px-4 py-2.5 text-xs font-black text-titulo shadow-sm transition hover:-translate-y-0.5 hover:bg-fondo-panel active:scale-[0.98]">
                <i class="ph-bold ph-upload-simple text-lg"></i>
                Subir Documento
            </button>
            <a href="#" class="inline-flex items-center justify-center gap-2 rounded-xl bg-boton-principal px-4 py-2.5 text-xs font-black text-inverso shadow-sm transition hover:-translate-y-0.5 hover:bg-fondo-panel active:scale-[0.98]">
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-[24px] border border-borde bg-fondo-card/95 p-6 shadow-sm flex flex-col justify-center text-center">
            <p class="text-[10px] font-black uppercase tracking-[0.18em] text-apoyo">Total Documentos</p>
            <p class="mt-2 text-3xl font-black text-titulo">{{ collect($documentosActivos)->count() }}</p>
        </div>

        <div class="rounded-[24px] border border-borde bg-fondo-card/95 p-6 shadow-sm flex flex-col justify-center text-center">
            <p class="text-[10px] font-black uppercase tracking-[0.18em] text-apoyo">Archivos Activos</p>
        ...
        <div class="rounded-[24px] border border-borde bg-fondo-card/95 p-6 shadow-sm flex flex-col justify-center text-center">
            <p class="text-[10px] font-black uppercase tracking-[0.18em] text-apoyo">Pendientes/Expirados</p>
            <p class="mt-2 text-3xl font-black text-amber-600">0</p>
        </div>

        <div class="rounded-[24px] border border-borde bg-fondo-card/95 p-6 shadow-sm flex flex-col justify-center text-center">
            <p class="text-[10px] font-black uppercase tracking-[0.18em] text-apoyo">Último Cargado</p>
    ...
    <div class="rounded-[24px] border border-borde bg-fondo-card/95 p-6 shadow-sm">
        <h3 class="text-lg font-black text-titulo border-b border-borde pb-4 mb-4">Documentos Recientes</h3>
                ...
                @foreach(collect($documentosActivos)->sortByDesc('fecha_subida')->take(3) as $doc)
                    <div class="rounded-xl border border-borde bg-fondo-panel p-4 flex items-start gap-3 transition hover:border-borde-fuerte hover:bg-fondo-panel">
                        ...
                        <div class="overflow-hidden">
                            <p class="text-sm font-black text-titulo truncate" title="{{ $doc->titulo }}">{{ $doc->titulo }}</p>
                            ...
                            <a href="{{ Storage::url($doc->ruta_archivo) }}" class="inline-block mt-2 text-xs font-black text-parrafo hover:underline">Ver / Descargar</a>
                        </div>
                    </div>
                @endforeach
            </div>
            <div class="mt-5 text-center">
                <button type="button" @click="abrir('documento')" class="inline-flex items-center justify-center gap-2 rounded-xl bg-fondo-card border border-borde-fuerte px-6 py-2.5 text-xs font-black uppercase tracking-wider text-titulo shadow-sm transition hover:bg-fondo-panel active:scale-[0.98]">
        ...
        @else
            <div class="py-8 text-center">
                ...
                <button type="button" @click="abrir('documento')" class="mt-4 inline-flex items-center justify-center gap-2 rounded-xl bg-boton-principal px-5 py-2.5 text-xs font-black text-inverso shadow-sm transition hover:-translate-y-0.5 hover:bg-fondo-panel">
