...
<section class="overflow-hidden rounded-[24px] border border-borde bg-fondo-panel shadow-sm backdrop-blur-xl mb-4">
    ...
    <div class="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div class="flex items-center gap-4">
            ...
            @endif
            <div>
                <h1 class="text-xl font-black text-titulo leading-tight">{{ $nombreCompleto ?: 'Sin nombre' }}</h1>
                <div class="mt-1 flex flex-wrap items-center gap-2 text-xs font-bold text-apoyo">
                    ...
                    @endphp
                    <span class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-black uppercase tracking-wide {{ $isExpedienteCompleto ? 'bg-fondo-panel text-parrafo' : 'bg-amber-600/20 text-amber-700' }}">
        ...
        <div class="flex flex-wrap items-center gap-2">
            <a href="{{ route('admin.adultos-mayores.index') }}" class="rounded-xl border border-borde-suave bg-fondo-card/50 px-4 py-2 text-xs font-black transition hover:bg-fondo-card text-titulo">
                <i class="ph-bold ph-arrow-left"></i> Volver
            </a>
            @if($idAdulto)
                <a href="{{ route('admin.adultos-mayores.edit', $idAdulto) }}" class="rounded-xl bg-fondo-card px-4 py-2 text-xs font-black text-titulo border border-borde-suave shadow-sm transition hover:bg-fondo-panel">
                    <i class="ph-bold ph-pencil-simple"></i> Editar datos
                </a>
            @endif
            <a href="{{ route('admin.adultos-mayores.reporte-individual', $idAdulto) }}" class="rounded-xl bg-boton-principal px-4 py-2 text-xs font-black text-inverso shadow-sm transition hover:bg-fondo-panel">
