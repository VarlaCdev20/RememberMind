...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-borde pb-3">
        <div>
            ...
            <p class="text-xs text-apoyo mt-0.5">Control evolutivo de constantes vitales y parámetros fisiológicos.</p>
        </div>
    ...
    @if($ultimo)
        <div class="rounded-3xl border border-borde bg-fondo-panel p-5 shadow-sm">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 border-b border-borde pb-3 mb-4">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs text-parrafo font-medium">
                        {{ $ultimo->fecha ? \Carbon\Carbon::parse($ultimo->fecha)->format('d/m/Y') : '' }} · {{ substr($ultimo->hora ?? '00:00', 0, 5) }}
                    </span>
                </div>
                <div class="text-xs text-apoyo">
                    Registrado por: <strong class="text-parrafo">{{ $ultimo->profesional->name ?? 'Equipo de Enfermería' }}</strong>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-8">
                {{-- 1. PA --}}
                <div class="rounded-2xl bg-fondo-card p-3 border border-borde text-center">
                    <span class="text-[10px] font-bold text-apoyo uppercase block">Presión Art.</span>
                    <span class="text-base font-black text-titulo mt-1 block">
                        {{ $ultimo->presion_arterial ?: '--' }}
                    </span>
                    <span class="text-[10px] text-apoyo">mmHg</span>
                </div>

                {{-- 2. FC --}}
                <div class="rounded-2xl bg-fondo-card p-3 border border-borde text-center">
                    <span class="text-[10px] font-bold text-apoyo uppercase block">Frec. Cardaca</span>
                    <span class="text-base font-black text-titulo mt-1 block">
                        {{ $ultimo->frecuencia_cardiaca ?: '--' }}
                    </span>
                    <span class="text-[10px] text-apoyo">lpm</span>
                </div>

                {{-- 3. FR --}}
                <div class="rounded-2xl bg-fondo-card p-3 border border-borde text-center">
                    <span class="text-[10px] font-bold text-apoyo uppercase block">Frec. Resp.</span>
                    <span class="text-base font-black text-titulo mt-1 block">
                        {{ $ultimo->frecuencia_respiratoria ?: '--' }}
                    </span>
                    <span class="text-[10px] text-apoyo">rpm</span>
                </div>

                {{-- 4. Temperatura --}}
                <div class="rounded-2xl bg-fondo-card p-3 border border-borde text-center">
                    <span class="text-[10px] font-bold text-apoyo uppercase block">Temperatura</span>
                    <span class="text-base font-black text-titulo mt-1 block">
                        {{ $ultimo->temperatura ? $ultimo->temperatura . '°' : '--' }}
                    </span>
                    <span class="text-[10px] text-apoyo">°C</span>
                </div>

                {{-- 5. SpO2 --}}
                <div class="rounded-2xl bg-fondo-card p-3 border border-borde text-center">
                    <span class="text-[10px] font-bold text-apoyo uppercase block">Saturación O2</span>
                    <span class="text-base font-black text-titulo mt-1 block">
                        {{ $ultimo->saturacion ? $ultimo->saturacion . '%' : '--' }}
                    </span>
                    <span class="text-[10px] text-apoyo">SpO2</span>
                </div>

                {{-- 6. Glucosa --}}
                <div class="rounded-2xl bg-fondo-card p-3 border border-borde text-center">
                    <span class="text-[10px] font-bold text-apoyo uppercase block">Glucemia</span>
                    <span class="text-base font-black text-titulo mt-1 block">
                        {{ $ultimo->glucosa ?: '--' }}
                    </span>
                    <span class="text-[10px] text-apoyo">mg/dL</span>
                </div>

                {{-- 7. Peso --}}
                <div class="rounded-2xl bg-fondo-card p-3 border border-borde text-center">
                    <span class="text-[10px] font-bold text-apoyo uppercase block">Peso Corporal</span>
                    <span class="text-base font-black text-titulo mt-1 block">
                        {{ $ultimo->peso ? $ultimo->peso . ' kg' : '--' }}
                    </span>
                    <span class="text-[10px] text-apoyo">kg</span>
                </div>

                {{-- 8. Dolor --}}
                <div class="rounded-2xl bg-fondo-card p-3 border border-borde text-center">
                    <span class="text-[10px] font-bold text-apoyo uppercase block">Dolor (EVA)</span>
                    ...
                    <span class="text-[10px] text-apoyo">Escala 0-10</span>
                </div>
            </div>

            @if($ultimo->observaciones)
                <div class="mt-3 rounded-xl bg-fondo-card p-3 border border-borde text-xs text-parrafo">
    ...
    @else
        <div class="rounded-3xl border border-borde bg-fondo-panel p-8 text-center">
            <i class="ph-bold ph-heartbeat text-3xl text-apoyo mb-2 block"></i>
            <p class="text-sm font-bold text-titulo">No existen controles de signos vitales registrados.</p>
            <p class="text-xs text-apoyo mt-1">Utilice el botón superior para ingresar la primera medición del residente.</p>
        </div>
    @endif
    ...
    <div class="rounded-3xl border border-borde bg-fondo-panel p-5 shadow-sm space-y-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-borde pb-3">
            <div>
                <h3 class="text-xs font-bold text-titulo flex items-center gap-1.5">
                    <i class="ph-bold ph-chart-line-up text-blue-600"></i>
                    <span>Evolución Temporal de Parámetros Clínicos</span>
                </h3>
                <p class="text-[11px] text-apoyo">Visualice la trayectoria hemodinámica según la métrica seleccionada.</p>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-1.5">
                @foreach([
                    ...
                    <button type="button"
                            ...
                            class="rounded-lg px-2.5 py-1 text-xs font-bold transition {{ $metricaSignosSeleccionada === $metKey ? 'bg-blue-600 text-white shadow-sm' : 'bg-fondo-card text-parrafo hover:bg-borde' }}">
                        {{ $metLabel }}
                    </button>
                @endforeach
            </div>
        ...
        @if($totalPuntos >= 2)
            ...
            <div class="rounded-2xl bg-fondo-card p-4 border border-borde">
                <div class="flex items-center justify-between mb-2 text-xs">
                    ...
                    <span class="text-[11px] text-apoyo font-medium">Últimas {{ $ptsCount }} tomas registradas</span>
                </div>
                ...
                <div class="mt-2 flex items-center justify-between text-[11px] text-apoyo font-medium border-t border-borde pt-2">
        ...
        @else
            <div class="rounded-2xl bg-fondo-card p-6 border border-borde text-center">
                <i class="ph-bold ph-chart-line text-2xl text-apoyo mb-2 block"></i>
                <p class="text-xs font-bold text-parrafo">No existen suficientes registros para mostrar una tendencia.</p>
                <p class="text-[11px] text-apoyo mt-1">Se requieren al menos 2 mediciones reales para graficar la evolución de {{ $metricaSignosSeleccionada }}.</p>
            </div>
        @endif
    </div>
    ...
    <div class="rounded-3xl border border-borde bg-fondo-panel p-5 shadow-sm space-y-3">
        <div class="flex items-center justify-between border-b border-borde pb-3">
            ...
            <span class="text-xs text-apoyo font-medium">Total: {{ $adultoMayor->signosVitales->count() }} registros</span>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-xs">
                ...
                <tbody class="divide-y divide-borde/50">
                    @forelse($adultoMayor->signosVitales as $sig)
                        <tr class="hover:bg-fondo-card/60 transition">
                            <td class="py-2.5 px-3 whitespace-nowrap font-medium text-titulo">
                                ...
                                <span class="text-apoyo block text-[10px]">{{ substr($sig->hora ?? '00:00', 0, 5) }}</span>
                            </td>
                            <td class="py-2.5 px-3 font-bold text-titulo whitespace-nowrap">{{ $sig->presion_arterial ?: '--' }}</td>
                            ...
                            <td class="py-2.5 px-3 max-w-[200px] truncate text-parrafo" title="{{ $sig->observaciones }}">
                                {{ $sig->observaciones ?: '--' }}
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="10" class="py-6 text-center text-xs text-apoyo italic">
