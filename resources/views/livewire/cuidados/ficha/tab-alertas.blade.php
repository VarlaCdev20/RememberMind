    ...
    <div class="rounded-3xl border border-borde rm-surface-card bg-fondo-panel p-5 shadow-sm space-y-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 border-b border-borde pb-3">
            <div>
                <h2 class="text-xs font-black uppercase tracking-wider text-titulo flex items-center gap-2">
                    ...
                    <span>Alertas ClÃ­nicas Activas</span>
                </h2>
                <p class="text-xs text-apoyo mt-0.5">Situaciones de vigilancia prioritaria que requieren intervenciÃ³n asistencial.</p>
            </div>
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                            <div class="space-y-1.5 min-w-0">
                                ...
                                <p class="text-xs font-medium text-parrafo leading-relaxed">
                                    {{ $al->motivo ?? 'Alerta generada por parÃ¡metros fuera de rango' }}
                                </p>
                                ...
                                <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-apoyo pt-1">
                                    ...
                                    <span>Â·</span>
                                    <span>Origen: <strong>{{ $al->origen ?? 'Sistema ClÃ­nico' }}</strong></span>
                                    @if($al->responsable)
                                        <span>Â·</span>
                                    ...
                                    <div class="mt-2 rounded-xl bg-white/80 p-2.5 border border-borde text-xs space-y-1">
                                        ...
                                        @foreach($al->acciones as $acc)
                                            <p class="text-parrafo text-[11px]">
                                                Â· {{ $acc->accion }} <span class="text-apoyo">({{ $acc->fecha_accion?->format('H:i') ?? $acc->created_at?->format('H:i') }})</span>
                                            </p>
                                        @endforeach
                                    </div>
                                @endif
                            ...
                            <div class="shrink-0 pt-1">
                                ...
                                @elseif($esEnAtencion)
                                    <button type="button"
                                        ...
                                        <span>Registrar acciÃ³n / Cerrar</span>
                                    </button>
                                @endif
                            </div>
                        </div>
        ...
        @else
            <div class="rounded-2xl bg-emerald-50/40 p-6 border border-emerald-200 text-center">
                ...
                <p class="text-xs font-bold text-emerald-900">Sin alertas clÃ­nicas activas</p>
                <p class="text-[11px] text-emerald-700 mt-0.5">El residente se encuentra hemodinÃ¡micamente estable y sin condiciones crÃ­ticas abiertas.</p>
            </div>
        @endif
    </div>

    {{-- 2. HISTORIAL DE ALERTAS CERRADAS (HISTÃ“RICO AUDITADO) --}}
    <div class="rounded-3xl border border-borde rm-surface-card bg-fondo-panel p-5 shadow-sm space-y-4">
            ...
            <table class="w-full text-left text-xs">
                <thead>
                    <tr class="border-b border-borde text-[10px] font-black uppercase text-apoyo">
                        ...
                        <th class="py-2.5 px-3">ResoluciÃ³n / Acciones Realizadas</th>
                    ...
                    @forelse($alertasCerradas as $alc)
                        <tr class="hover:bg-fondo-card/60 transition">
                            ...
                            <td class="py-3 px-3 text-parrafo max-w-[260px]">
                                ...
                                @if($alc->acciones && $alc->acciones->count() > 0)
                                    <span class="text-[10px] text-apoyo block truncate">
                                        AcciÃ³n: {{ $alc->acciones->last()->accion ?? 'AtenciÃ³n oportuna' }}
                                    </span>
                                @else
                                    <span class="text-[10px] text-apoyo">Cerrada tras resoluciÃ³n clÃ­nica</span>
                                @endif
                            </td>
                            ...
                            <td class="py-3 px-3 whitespace-nowrap text-apoyo font-medium">
                                {{ $alc->cerradoPor?->name ?? $alc->responsable?->name ?? 'EnfermerÃ­a' }}
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="py-8 text-center text-xs text-apoyo italic">
                                No se registran alertas histÃ³ricas cerradas para este residente.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
