<div class="space-y-5">
    ...
    <div class="space-y-3">
        @forelse($tareas as $tarea)
            <article wire:key="tarea-{{ $tarea->cod_ejecucion }}" class="rounded-2xl border border-borde bg-fondo-panel p-4 shadow-sm">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div class="min-w-0">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            <span class="rounded-full border border-borde-suave bg-fondo-base px-2 py-0.5 text-[10px] font-bold text-apoyo">Prioridad {{ mb_strtolower($tarea->intervencion?->prioridad ?? 'normal') }}</span>
                        </div>
                ...
                <dl class="mt-3 grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-4">
                    <div><dt class="text-xs font-bold text-apoyo">Programación</dt><dd class="text-titulo">{{ $tarea->fecha_hora_programada?->format('d/m/Y H:i') ?? 'Sin fecha' }}</dd></div>
                    <div><dt class="text-xs font-bold text-apoyo">Turno</dt><dd class="text-titulo">{{ $tarea->turno?->nombre ?? 'Sin turno' }}</dd></div>
                    <div><dt class="text-xs font-bold text-apoyo">Área</dt><dd class="text-titulo">{{ $tarea->intervencion?->plan?->area?->nombre ?? 'Sin área' }}</dd></div>
                    <div><dt class="text-xs font-bold text-apoyo">Responsable</dt><dd class="text-titulo">{{ $tarea->responsable?->nombre_completo ?? 'Sin asignar' }}</dd></div>
                </dl>
