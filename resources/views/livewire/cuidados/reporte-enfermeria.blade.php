<div class="rm-page-layout font-sans space-y-5">
    ...
    <section class="rm-filter-bar p-3.5 grid gap-3 md:grid-cols-3">
        ...
        <label class="text-xs font-bold text-titulo">Residente<select wire:model.live="codResidente" class="mt-1 rm-input w-full text-xs"><option value="">{{ $esSuperAdmin ? 'Todos los residentes' : 'Todos los asignados' }}</option>@foreach($pacientes as $p)<option value="{{ $p->cod_residente }}">{{ $p->ap_paterno }}, {{ $p->nombres }}</option>@endforeach</select></label>
    </section>
    ...
    <div class="grid gap-5 lg:grid-cols-2">
        <section class="rm-surface-card bg-fondo-panel rounded-2xl border border-borde p-4 sm:p-5 shadow-sm"><h2 class="text-lg font-black text-titulo">Cuidados registrados</h2><div class="mt-4 max-h-[32rem] space-y-2 overflow-y-auto">@forelse($cuidados as $r)<article class="rounded-xl bg-fondo-card p-3"><div class="flex justify-between gap-3"><p class="text-xs font-black text-titulo">{{ $r->adultoMayor?->nombres }} {{ $r->adultoMayor?->apellido_paterno }} · {{ $r->intervencion?->nombre ?? 'Cuidado' }}</p><time class="text-[11px] text-apoyo">{{ ($r->fecha_hora_ejecucion ?? $r->fecha_hora_programada)?->format('d/m H:i') }}</time></div><p class="mt-1 text-xs text-parrafo">{{ $r->observacion ?: ($r->intervencion?->descripcion ?? 'Sin observación') }}</p></article>@empty<p class="text-sm text-apoyo">Sin actividad en el periodo.</p>@endforelse</div></section>
        <section class="rm-surface-card bg-fondo-panel rounded-2xl border border-borde p-4 sm:p-5 shadow-sm"><h2 class="text-lg font-black text-titulo">Incidentes y seguimiento</h2><div class="mt-4 max-h-[32rem] space-y-2 overflow-y-auto">@forelse($incidentes as $i)<article class="rounded-xl border border-estado-advertenciaBorde bg-estado-advertenciaBg p-3"><div class="flex justify-between gap-3"><p class="text-xs font-black text-titulo">{{ $i->adultoMayor?->nombres }} · {{ str_replace('_',' ',$i->tipo) }}</p><time class="text-[11px] text-apoyo">{{ $i->fecha_hora->format('d/m H:i') }}</time></div><p class="mt-1 text-xs text-parrafo">{{ $i->descripcion }}</p></article>@empty<p class="text-sm text-apoyo">Sin incidentes en el periodo.</p>@endforelse</div></section>
    </div>
</div>
