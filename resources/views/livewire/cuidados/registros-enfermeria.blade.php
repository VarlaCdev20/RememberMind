<div class="rm-pilot-enfermeria rm-page-layout font-sans space-y-5">
    ...
    <section class="rounded-2xl border border-[var(--rm-border)] bg-[var(--rm-surface)] p-4 shadow-sm">
        ...
        <select id="residente-registro" wire:model.live="codResidente" class="mt-2 w-full rounded-xl border border-[var(--rm-border)] bg-[var(--rm-surface-alt)] px-4 py-2.5 text-sm text-[var(--rm-text-body)] outline-none focus:border-[var(--rm-border)]-focus">
        ...
        @error('codResidente') <p class="mt-1 text-xs font-bold text-estado-peligro">{{ $message }}</p> @enderror
    </section>
                ...
                <div class="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    ...
                    @if(!in_array($tipo, ['ALIMENTACION','HIDRATACION','ELIMINACION','MOVILIDAD','SUENO']))
                        <label class="text-xs font-bold text-[var(--rm-text-title)] md:col-span-2">Intervención del plan de cuidados
                            <select wire:model="intervencionId" class="mt-1 w-full rounded-xl border border-[var(--rm-border)] bg-[var(--rm-surface-alt)] px-3 py-2.5 text-sm">
                            ...
                            @error('intervencionId') <span class="text-xs text-estado-peligro">{{ $message }}</span> @enderror
                        </label>
                    @endif
        ...
        @elseif($seccion === 'DOLOR')
            <form wire:submit="guardarDolor" class="rounded-2xl border border-[var(--rm-border)] bg-[var(--rm-surface)] p-5 shadow-sm">
                ...
                <div class="mt-4 grid gap-4 md:grid-cols-2">
                    ...
                    @if($faseDolor !== 'VALORACION')<label class="text-xs font-bold text-[var(--rm-text-title)] md:col-span-2">Valoración inicial<select wire:model="valoracionDolorId" class="mt-1 w-full rounded-xl border border-[var(--rm-border)] bg-[var(--rm-surface-alt)] px-3 py-2.5"><option value="">Seleccione</option>@foreach($adulto->valoracionesDolor as $r)<option value="{{ $r->cod_valoracion_dolor }}">{{ $r->fecha_hora?->format('d/m H:i') ?? 'N/A' }} — dolor {{ $r->intensidad }}/10</option>@endforeach</select></label>@endif
            ...
            @if($adulto->incidentesClinicos->whereIn('estado', ['ABIERTO','EN_SEGUIMIENTO'])->isNotEmpty())
                <div class="mt-5 grid gap-5 lg:grid-cols-2">
                    <form wire:submit="seguimientoIncidente" class="rounded-2xl border border-[var(--rm-border)] bg-[var(--rm-surface)] p-5 shadow-sm">
                        ...
                        <select wire:model="incidenteId" class="mt-3 w-full rounded-xl border border-[var(--rm-border)] bg-[var(--rm-surface-alt)] px-3 py-2.5"><option value="">Seleccione</option>@foreach($adulto->incidentesClinicos->whereIn('estado',['ABIERTO','EN_SEGUIMIENTO']) as $i)<option value="{{ $i->cod_incidente }}">{{ $i->tipo }} · {{ $i->fecha_hora_evento->format('d/m H:i') }}</option>@endforeach</select>
                    ...
                    <form wire:submit="cerrarIncidente" class="rounded-2xl border border-[var(--rm-border)] bg-[var(--rm-surface)] p-5 shadow-sm">
                        ...
                        <select wire:model="incidenteId" class="mt-3 w-full rounded-xl border border-[var(--rm-border)] bg-[var(--rm-surface-alt)] px-3 py-2.5"><option value="">Seleccione</option>@foreach($adulto->incidentesClinicos->whereIn('estado',['ABIERTO','EN_SEGUIMIENTO']) as $i)<option value="{{ $i->cod_incidente }}">{{ $i->tipo }} · {{ $i->estado }}</option>@endforeach</select>
