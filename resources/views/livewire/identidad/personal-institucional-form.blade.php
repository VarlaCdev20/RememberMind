...
@if(!$esEdicion)
<div class="p-3 md:p-4 bg-[#F0E8DE] flex flex-col h-full rounded-xl shadow-xl overflow-hidden">
    <!-- Header Compacto -->
                        ...
                        $hasError 
                            ...
                            : ($isActive 
                                ...
                                : ($isCompleted 
                                    ...
                                    : 'bg-[#FBF8F5] text-apoyo/40 border-2 border-[#D5CABE] hover:border-[#A35A44]/30'))
                    }}">
                        @if($hasError)
    ...
    ">
                ...
                <div class="space-y-2">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                        ...
                        <div class="md:col-span-1 flex flex-col items-center justify-center rounded-lg p-4 bg-fondo/30 border border-borde text-center">
                            ...
                            <p class="text-[10px] text-apoyo bg-[#FBF8F5] px-3 py-1.5 rounded-full border border-[#D5CABE] shadow-sm inline-block">JPG, PNG o WEBP <span class="font-bold text-estado-peligro/70">(Máx. 2MB)</span></p>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 bg-yellow-50/50 p-4 rounded-xl border border-yellow-200">
                                    @if($zona_id === 'OTRA')
                                        <div>
                                            ...
                                            <input type="text" wire:model.blur="otra_zona" class="w-full rounded-xl border-yellow-300 bg-[#FBF8F5] text-sm uppercase focus:ring-yellow-400 focus:border-yellow-400" placeholder="Ej: VILLA ARMONÍA">
                                    ...
                                    @if($calle_id === 'OTRA')
                                        <div>
                                            ...
                                            <input type="text" wire:model.blur="otra_calle" class="w-full rounded-xl border-yellow-300 bg-[#FBF8F5] text-sm uppercase focus:ring-yellow-400 focus:border-yellow-400" placeholder="Ej: CALLE 15 DE SEPTIEMBRE">
                ...
                <div class="space-y-2 animate-fade-in">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        @foreach($roles as $rol)
                            <label class="flex items-center gap-2 p-2.5 border rounded-xl cursor-pointer hover:bg-fondo-hover transition-colors {{ $rol->name === $rol_seleccionado ? 'border-boton-acento bg-boton-acento/5 shadow-sm' : 'border-borde bg-[#FBF8F5]' }}">
                        ...
                        @if($clsf)
                            ...
                            <div class="p-4 {{ $clsf['tipo_personal'] === 'salud' ? 'bg-estado-infoBg border-estado-infoBorde' : 'bg-estado-advertenciaBg border-estado-advertenciaBorde' }} border rounded-xl space-y-2 animate-fade-in">
                                <div class="flex items-center justify-between gap-2">
                                    ...
                                    <span class="shrink-0 inline-flex items-center gap-1 rounded-full bg-[#FBF8F5] px-2.5 py-1.5 text-[9px] font-black text-titulo border border-borde shadow-sm uppercase tracking-wider">
                                        <i class="ph-bold ph-check-circle text-boton-acento"></i> Auto
                                    </span>
                                </div>
                                <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                    <div class="rounded-xl border border-borde bg-[#FBF8F5] p-2.5 shadow-sm">
                                    ...
                                    <div class="rounded-xl border border-borde bg-[#FBF8F5] p-2.5 shadow-sm col-span-2 sm:col-span-1">
                    ...
                    @else
                        ...
                        <div class="space-y-2">
                            @forelse($docsInstitucionales as $doc)
                                ...
                                @endphp
                                <div wire:key="doc-inst-{{ $doc['id'] }}" class="flex items-center justify-between p-2.5 border border-[#D5CABE] rounded-xl bg-[#FBF8F5] shadow-sm transition-all {{ $instSubido ? 'border-estado-exito/30' : ($instObservado ? 'border-estado-peligro/30 bg-estado-peligroBg/10' : 'border-borde hover:border-boton-acento/40') }}">
                                    <div class="flex items-center gap-2 overflow-hidden flex-1">
            ...
            @elseif($pasoActual == 7)
                ...
                <div class="space-y-2">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-left">
                        <!-- Sección: Acceso -->
                        <div class="bg-[#F0E8DE] border border-[#D5CABE] rounded-[1.25rem] p-5 shadow-sm space-y-2">
                        ...
                        <div class="bg-[#F0E8DE] border border-[#D5CABE] rounded-[1.25rem] p-3 md:p-4 shadow-sm space-y-2 col-span-1 md:col-span-2">
                            <div class="flex items-center gap-2 pb-3 border-b border-borde/50">
                                <div class="w-7 h-7 rounded-full bg-boton-acento/10 text-boton-acento flex items-center justify-center">
        ...
        <div class="flex items-center justify-between pt-3 mt-2 border-t border-[#D5CABE] bg-[#F0E8DE] sticky bottom-0 z-10 pb-1">
            @if($pasoActual > 1)
        ...
        <div class="space-y-2">
            ...
            @if($clasificacion)
                <div class="p-4 {{ $clasificacion['tipo_personal'] === 'salud' ? 'bg-estado-infoBg border-estado-infoBorde' : 'bg-estado-advertenciaBg border-estado-advertenciaBorde' }} border rounded-xl space-y-2">
                    ...
                    <div class="space-y-2">
                        <div class="rounded-xl border border-borde bg-[#FBF8F5] p-3">
...
{!! file_get_contents(resource_path('frontend/scripts/modules/livewire-identidad-personal-institucional-form.js')) !!}
</script>
@endscript
