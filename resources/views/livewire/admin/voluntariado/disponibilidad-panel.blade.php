    ...
    <div class="mx-auto max-w-7xl space-y-5">
                ...
                <div class="flex flex-wrap gap-2">
                    @can('voluntarios.crear')
                        <button
                            ...
                            class="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-boton-acento px-4 text-xs font-black uppercase tracking-wide text-inverso shadow-[0_8px_18px_rgba(226,125,96,0.22)] transition hover:-translate-y-0.5 hover:bg-fondo-panel active:scale-95"
                        >
                    ...
                    <a href="{{ $linksCabecera['voluntarios'] }}" class="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-borde-suave bg-fondo-panel px-4 text-xs font-black uppercase tracking-wide text-titulo shadow-sm transition hover:border-borde-focus hover:text-boton-acento active:scale-95">
                        <i class="ph-bold ph-users-three text-sm"></i>
                        Ver voluntarios
                    </a>

                    <a href="{{ $linksCabecera['resumen'] }}" class="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-borde-suave bg-fondo-panel px-4 text-xs font-black uppercase tracking-wide text-titulo shadow-sm transition hover:bg-fondo-panel active:scale-95">
            ...
            @foreach($metricas as $metrica)
                ...
                <article class="relative min-h-[104px] overflow-hidden rounded-2xl border border-borde-suave bg-fondo-panel p-3.5 shadow-sm backdrop-blur-xl transition duration-300 hover:-translate-y-0.5 hover:border-borde-focus">
                    ...
                    <div class="flex items-start justify-between gap-2">
                        <p class="text-xs font-black uppercase leading-snug tracking-[0.12em] text-apoyo">{{ $metrica['label'] }}</p>
        ...
        <section class="rounded-[1.35rem] border border-borde-suave bg-fondo-panel p-3.5 shadow-sm backdrop-blur-xl">
            <div class="grid gap-3 lg:grid-cols-[1.3fr_0.75fr_0.75fr_0.75fr_0.75fr_auto_auto]">
                <label class="block">
                    <span class="mb-1 block text-xs font-black uppercase tracking-widest text-apoyo">Buscar voluntario</span>
                ...
                <label class="block">
                    <span class="mb-1 block text-xs font-black uppercase tracking-widest text-apoyo">Fecha</span>
                ...
                <label class="block">
                    <span class="mb-1 block text-xs font-black uppercase tracking-widest text-apoyo">Día</span>
                ...
                <label class="block">
                    <span class="mb-1 block text-xs font-black uppercase tracking-widest text-apoyo">Turno</span>
                ...
                <label class="block">
                    <span class="mb-1 block text-xs font-black uppercase tracking-widest text-apoyo">Estado</span>
                ...
                <div class="flex items-end">
                    <button type="button" wire:click="$refresh" class="inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-boton-principal px-4 text-xs font-black uppercase tracking-wide text-inverso transition hover:bg-boton-acento active:scale-95 lg:w-auto">
                ...
                <div class="flex items-end">
                    <button type="button" wire:click="limpiarFiltros" class="inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-borde-suave bg-fondo-panel px-4 text-xs font-black uppercase tracking-wide text-titulo transition hover:bg-fondo-panel active:scale-95 lg:w-auto">
        ...
        <section class="grid gap-5 xl:grid-cols-[1.9fr_1fr]">
            <div class="rounded-[1.45rem] border border-borde-suave bg-fondo-panel p-4 shadow-sm backdrop-blur-xl">
                <div class="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <span class="text-xs font-black uppercase tracking-[0.15em] text-boton-acento">Calendario de disponibilidad</span>
                        <h2 class="mt-0.5 text-base font-black text-titulo">{{ $rangoSemana }}</h2>
                    </div>
                    <div class="flex gap-1.5">
                        ...
                        <button type="button" wire:click="irHoy" class="inline-flex h-9 items-center justify-center rounded-xl border border-borde-suave bg-fondo-panel px-3 text-xs font-black uppercase tracking-wide text-titulo transition hover:bg-fondo-panel">
                            Hoy
                        </button>
                    ...
                    @foreach($calendario as $dia)
                        <article class="min-h-[168px] rounded-2xl border border-borde-suave bg-fondo-panel p-3">
                            <header class="mb-2 flex items-start justify-between gap-2">
                                <div>
                                    <h3 class="text-xs font-black uppercase tracking-wider text-titulo">{{ $dia['dia'] }}</h3>
                                    <p class="text-xs font-bold text-apoyo">{{ $dia['fecha']->format('d/m') }}</p>
                                </div>
                                <span class="rounded-full bg-fondo-panel px-2 py-0.5 text-xs font-black text-apoyo">{{ $dia['items']->count() }}</span>
                            </header>
                            ...
                            <div class="space-y-1.5">
                                ...
                                @if($dia['items']->count() > 4)
                                    <p class="pt-1 text-center text-xs font-black uppercase tracking-wide text-apoyo">
            ...
            <aside class="rounded-[1.45rem] border border-borde-suave bg-fondo-panel p-4 shadow-sm backdrop-blur-xl">
                <div class="mb-3 flex items-center justify-between gap-3">
                    <div>
                        <span class="text-xs font-black uppercase tracking-[0.15em] text-boton-acento">Listado breve</span>
                        <h2 class="mt-0.5 text-base font-black text-titulo">Horarios registrados</h2>
                    </div>
                    ...
                    @forelse($registros as $registro)
                        ...
                        <article class="rounded-2xl border border-borde-suave bg-fondo-panel p-3">
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                                    <p class="truncate text-xs font-black text-titulo">{{ $registro->nombre_voluntario ?: 'Voluntario' }}</p>
        ...
        <div class="fixed inset-0 z-[90] flex items-center justify-center px-4 py-6">
            ...
            <section class="relative flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-[1.5rem] border border-borde-suave bg-fondo-app shadow-[0_24px_70px_rgba(47,62,92,0.28)]">
                <header class="flex items-start justify-between gap-4 border-b border-borde-suave bg-fondo-panel px-5 py-4">
                    <div>
                        <span class="text-xs font-black uppercase tracking-[0.15em] text-boton-acento">{{ $isEdit ? 'Editar horario' : 'Nuevo horario' }}</span>
                        <h2 class="mt-1 text-xl font-black text-titulo">{{ $isEdit ? 'Editar disponibilidad' : 'Registrar disponibilidad' }}</h2>
                ...
                <div class="flex-1 overflow-y-auto p-5">
                    <div class="grid gap-4 sm:grid-cols-2">
                        <label class="block sm:col-span-2">
                            <span class="mb-1.5 block text-xs font-black uppercase tracking-widest text-apoyo">Voluntario</span>
                        ...
                        <label class="block">
                            <span class="mb-1.5 block text-xs font-black uppercase tracking-widest text-apoyo">Día</span>
                        ...
                        <label class="block">
                            <span class="mb-1.5 block text-xs font-black uppercase tracking-widest text-apoyo">Turno calculado</span>
                            <div class="flex h-11 items-center rounded-xl border border-borde-suave bg-fondo-panel px-3 text-sm font-black text-apoyo">
                                {{ $hora_inicio ? ($hora_inicio < '12:00' ? 'Mañana' : ($hora_inicio < '18:00' ? 'Tarde' : 'Noche')) : 'Flexible' }}
                            </div>
                        </label>

                        <label class="block">
                            <span class="mb-1.5 block text-xs font-black uppercase tracking-widest text-apoyo">Hora inicio</span>
                        ...
                        <label class="block">
                            <span class="mb-1.5 block text-xs font-black uppercase tracking-widest text-apoyo">Hora fin</span>
                        ...
                        <label class="block sm:col-span-2">
                            <span class="mb-1.5 block text-xs font-black uppercase tracking-widest text-apoyo">Observación</span>
                ...
                <footer class="flex flex-col-reverse gap-2 border-t border-borde-suave bg-fondo-panel px-5 py-4 sm:flex-row sm:justify-end">
                    <button type="button" wire:click="cerrarFormulario" class="inline-flex h-10 items-center justify-center rounded-xl border border-borde-suave bg-fondo-panel px-4 text-xs font-black uppercase tracking-wide text-titulo transition hover:bg-fondo-panel">
                        Cancelar
                    </button>
                    <button type="button" wire:click="guardar" class="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-boton-principal px-5 text-xs font-black uppercase tracking-wide text-inverso shadow-[0_8px_18px_rgba(47,62,92,0.18)] transition hover:bg-boton-acento active:scale-95">
