<div class="relative mx-auto max-w-7xl space-y-6 py-6 px-4 sm:px-6 lg:px-8">
    ...
    <section class="overflow-hidden rounded-[24px] border border-borde bg-fondo-panel shadow-[0_12px_28px_rgba(47,62,92,0.08)] backdrop-blur-xl">
        ...
        <div class="p-6">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    <span class="text-[11px] font-black uppercase tracking-[0.2em] text-parrafo">
                ...
                <div class="shrink-0">
                    <a href="{{ route('admin.adultos-mayores.index') }}" class="inline-flex items-center justify-center gap-2 rounded-xl bg-boton-principal px-4 py-2.5 text-xs font-black text-inverso shadow-md transition hover:bg-fondo-panel active:scale-95">
    ...
    <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-7">
        ...
        <div class="rounded-[18px] border border-borde-suave bg-fondo-panel p-4 shadow-xs">
            <p class="text-[9px] font-black uppercase tracking-[0.15em] text-apoyo">Total Alertas</p>
            <p class="mt-1.5 text-2xl font-black text-boton-acento">{{ $conteos['total'] }}</p>
        </div>
        {{-- Red de Apoyo --}}
        <div class="rounded-[18px] border border-borde-suave bg-fondo-panel p-4 shadow-xs">
            <p class="text-[9px] font-black uppercase tracking-[0.15em] text-apoyo">Red de Apoyo</p>
            <p class="mt-1.5 text-2xl font-black text-titulo">{{ $conteos['red_de_apoyo'] }}</p>
        </div>
        {{-- Documentos --}}
        <div class="rounded-[18px] border border-borde-suave bg-fondo-panel p-4 shadow-xs">
            <p class="text-[9px] font-black uppercase tracking-[0.15em] text-apoyo">Documentos</p>
            <p class="mt-1.5 text-2xl font-black text-titulo">{{ $conteos['documentacion'] }}</p>
        </div>
        {{-- Salud --}}
        <div class="rounded-[18px] border border-borde-suave bg-fondo-panel p-4 shadow-xs">
            <p class="text-[9px] font-black uppercase tracking-[0.15em] text-apoyo">Salud y Cuidados</p>
            <p class="mt-1.5 text-2xl font-black text-titulo">{{ $conteos['salud_y_cuidados'] }}</p>
        </div>
        {{-- Evaluaciones --}}
        <div class="rounded-[18px] border border-borde-suave bg-fondo-panel p-4 shadow-xs">
            <p class="text-[9px] font-black uppercase tracking-[0.15em] text-apoyo">Evaluaciones</p>
            <p class="mt-1.5 text-2xl font-black text-parrafo">{{ $conteos['evaluaciones'] }}</p>
        </div>
        {{-- Seguimiento --}}
        <div class="rounded-[18px] border border-borde-suave bg-fondo-panel p-4 shadow-xs">
            <p class="text-[9px] font-black uppercase tracking-[0.15em] text-apoyo">Seguimiento</p>
            <p class="mt-1.5 text-2xl font-black text-estado-exito">{{ $conteos['seguimiento'] }}</p>
        </div>
        {{-- Estado --}}
        <div class="rounded-[18px] border border-borde-suave bg-fondo-panel p-4 shadow-xs">
            <p class="text-[9px] font-black uppercase tracking-[0.15em] text-apoyo">Institucional</p>
    ...
    <section class="overflow-hidden rounded-[24px] border border-borde bg-fondo-panel p-4 shadow-[0_4px_14px_rgba(47,62,92,0.04)]">
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div class="flex flex-wrap items-center gap-1.5">
                <span class="text-[10px] font-black uppercase tracking-widest text-apoyo mr-1">Filtrar:</span>
                ...
                @foreach($filtros as $f)
                    <button
                        ...
                        class="rounded-lg px-3 py-1.5 text-[10px] font-black transition active:scale-95 {{ $filtroCategoria === $f['valor'] ? 'bg-boton-principal text-inverso shadow-xs' : 'bg-fondo-panel text-apoyo hover:bg-fondo-panel' }}"
                    >
                        {{ $f['label'] }}
                    </button>
                @endforeach
    ...
    <main class="space-y-4">
        @if($alertas->isEmpty())
            <div class="rounded-2xl border border-borde bg-fondo-panel p-16 text-center shadow-xs flex flex-col items-center justify-center min-h-[300px]">
                ...
                <h3 class="text-base font-black text-titulo">Sin alertas pendientes</h3>
                    ...
                    <article class="rounded-2xl border {{ $alerta['color_border'] }} {{ $alerta['color_bg'] }} p-5 shadow-xs transition hover:shadow-sm flex flex-col justify-between">
                        <div class="space-y-3">
                            ...
                            <div class="flex items-center justify-between gap-2">
                                <span class="text-[10px] font-black uppercase tracking-wider text-apoyo">
                                    {{ $alerta['adulto_id'] }}
                                </span>
                            ...
                            <div>
                                <h3 class="text-sm font-black text-titulo leading-snug">
                                    {{ $alerta['nombre'] }}
                                </h3>
                                <div class="flex items-center gap-1.5 mt-1">
                                    <span class="inline-flex items-center rounded-md bg-fondo-card/50 border border-borde px-2 py-0.5 text-[9px] font-black text-apoyo uppercase tracking-wide">
                            ...
                            @if($alerta['fecha'])
                                <p class="text-[10px] font-semibold text-apoyo flex items-center gap-1">
                        ...
                        <div class="mt-4 border-t border-borde pt-3 flex justify-end">
                            <a
                                ...
                                class="inline-flex items-center gap-1.5 rounded-lg bg-fondo-card/70 px-3.5 py-2 text-[10px] font-black uppercase tracking-wider text-titulo border border-borde transition hover:bg-fondo-card active:scale-95"
                            >
