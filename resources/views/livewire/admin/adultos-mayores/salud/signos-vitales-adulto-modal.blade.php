<div>
    ...
    <div class="rounded-[24px] border border-[#CBBBAA] bg-[#E7DDD2]/95 p-5 shadow-sm">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
                <h3 class="text-xl font-black text-[#2F3E5C]">Signos Vitales</h3>
                <p class="text-sm text-[#2F3E5C]/60">Registro hemodinámico, temperatura y métricas físicas.</p>
            </div>
            <button wire:click="abrirModalSignos('{{ $cod_am }}')" class="inline-flex items-center gap-2 rounded-xl bg-[#2F3E5C] px-4 py-2 text-sm font-bold text-white shadow-sm hover:bg-[#1f2937] active:scale-95 transition">
        ...
        @if(isset($signosList) && $signosList->isNotEmpty())
            <div class="mt-4 space-y-3">
                @foreach($signosList as $signo)
                    <div class="rounded-xl border border-[#C7B5A3] bg-white p-4 shadow-sm flex flex-col md:flex-row justify-between gap-4">
                        <div class="flex-1">
                            ...
                            @if($signo->imc)
                                <p class="text-xs font-bold mt-2 text-[#2F3E5C]">
                        ...
                        <div class="flex items-start shrink-0">
                            <button wire:click="abrirModalSignos('{{ $cod_am }}', {{ $signo->cod_signos_vitales }})" class="text-[#2F3E5C] bg-[#F8F2EC] hover:bg-[#D5C7B9] px-3 py-1.5 rounded-lg text-xs font-bold transition">
        ...
        @else
            <div class="mt-4 text-center border-2 border-dashed border-[#C7B5A3] rounded-xl p-4 bg-white/50">
                <p class="text-sm font-bold text-[#2F3E5C]/60">No hay registros de signos vitales recientes.</p>
            </div>
        @endif
    </div>
    ...
    <x-ui.modal-livewire wire:model="showModal" title="{{ $isEditing ? 'Editar Signos Vitales' : 'Registrar Signos Vitales' }}" maxWidth="3xl">
        ...
        <form wire:submit.prevent="guardar" id="formSignos">
            ...
            <div class="grid gap-6 md:grid-cols-2">
                ...
                <div class="rounded-xl border border-[#C7B5A3] bg-white/50 p-4 shadow-sm md:col-span-2">
                    <h4 class="mb-3 text-sm font-black uppercase text-[#2F3E5C]">1. Momento del Registro</h4>
                    <div class="grid gap-4 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="time" wire:model="hora"
                                class="w-full rounded-xl border {{ $errors->has('hora') ? 'border-terracota' : 'border-[#C7B5A3]' }} bg-[#F8F2EC] px-3 py-2 text-sm text-[#2F3E5C] focus:border-terracota focus:ring-terracota">
                ...
                <div class="rounded-xl border border-[#C7B5A3] bg-white/50 p-4 shadow-sm">
                    <h4 class="mb-3 text-sm font-black uppercase text-[#2F3E5C]">2. Hemodinámica</h4>
                    <div class="space-y-4">
                        <div>
                            <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-[#2F3E5C]/55">Presión Arterial (mmHg)</label>
                ...
                <div class="rounded-xl border border-[#C7B5A3] bg-white/50 p-4 shadow-sm">
                    <h4 class="mb-3 text-sm font-black uppercase text-[#2F3E5C]">3. Ventilación y Temperatura</h4>
                    <div class="space-y-4">
                        <div>
                ...
                <div class="rounded-xl border border-[#C7B5A3] bg-white/50 p-4 shadow-sm">
                    <h4 class="mb-3 text-sm font-black uppercase text-[#2F3E5C]">4. Mediciones y Antropometría</h4>
                    <div class="space-y-4">
                        <div>
                            <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-[#2F3E5C]/55">Glucosa (mg/dL)</label>
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="imc" readonly
                                class="w-full rounded-xl border border-transparent bg-[#D5C7B9]/30 px-3 py-2 text-sm font-bold text-[#2F3E5C]">
                        </div>
                    </div>
                </div>
                ...
                <!-- Dolor y Notas -->
                <div class="rounded-xl border border-[#C7B5A3] bg-white/50 p-4 shadow-sm">
                    <h4 class="mb-3 text-sm font-black uppercase text-[#2F3E5C]">5. Dolor y Observaciones</h4>
                    <div class="space-y-4">
                        <div>
                            <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-[#2F3E5C]/55">Escala de Dolor (0-10)</label>
                        ...
                        <div>
                            <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-[#2F3E5C]/55">Observación</label>
        ...
        <x-slot name="footer">
            ...
            <button type="submit" form="formSignos" class="inline-flex items-center justify-center gap-2 rounded-xl bg-terracota px-4 py-2 text-sm font-bold text-white transition hover:bg-[#C45F4B] active:scale-95 disabled:opacity-50" wire:loading.attr="disabled">
