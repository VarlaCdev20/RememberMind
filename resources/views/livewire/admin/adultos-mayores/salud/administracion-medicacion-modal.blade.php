    ...
    <x-ui.modal-livewire wire:model="showModal" title="Registrar Administración de Medicación" maxWidth="2xl">
        ...
        <form wire:submit.prevent="guardar" id="formAdministracion">
            ...
            <div class="mb-6 rounded-[1.4rem] border border-borde-suave bg-fondo-panel p-5 shadow-sm backdrop-blur-xl flex items-center justify-between">
                <div>
                    <span class="text-[10px] font-black uppercase tracking-widest text-boton-acento">Medicamento a Administrar</span>
                    <h3 class="mt-1 text-lg font-black text-titulo">{{ $medicamento_nombre ?: 'Sin Seleccionar' }}</h3>
            ...
            <div class="grid gap-6 md:grid-cols-2">
                ...
                <div class="rounded-[1.4rem] border border-borde-suave bg-fondo-panel p-5 shadow-sm md:col-span-2 backdrop-blur-xl">
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div>
                            <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-apoyo">Fecha de Toma *</label>
                        ...
                        @if($administrado)
                        <div>
                            <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-apoyo">Hora Real de Toma *</label>
                ...
                <div class="rounded-[1.4rem] border border-borde-focus bg-estado-peligroBg p-5 shadow-sm md:col-span-2 backdrop-blur-xl">
                    <h4 class="mb-4 flex items-center gap-2 text-sm font-black uppercase tracking-wider text-boton-acento">
                ...
                <div class="rounded-[1.4rem] border border-borde-suave bg-fondo-panel p-5 shadow-sm md:col-span-2 backdrop-blur-xl">
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        @if($administrado)
                        <div>
                            <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-apoyo">Efecto Observado</label>
                        ...
                        <div class="{{ $administrado ? '' : 'md:col-span-2' }}">
                            <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-apoyo">Nota / Responsable</label>
        ...
        <x-slot name="footer">
            ...
            <button type="submit" form="formAdministracion" class="inline-flex items-center justify-center gap-2 rounded-xl {{ $administrado ? 'bg-estado-exitoBg shadow-[0_8px_16px_rgba(141,162,128,0.2)] hover:bg-fondo-panel' : 'bg-boton-acento shadow-[0_8px_16px_rgba(226,125,96,0.2)] hover:bg-fondo-panel' }} px-5 py-2.5 text-xs font-black uppercase tracking-wider text-inverso transition hover:-translate-y-0.5 active:scale-95 disabled:opacity-50" wire:loading.attr="disabled">
