                    ...
                    @forelse($valoraciones as $val)
                        <tr class="transition hover:bg-fondo-app/50">
                            <td class="px-5 py-3 font-bold">
                                {{ $val->adultoMayor->nombre_completo ?? trim(($val->adultoMayor->nombres ?? 'S/D') . ' ' . ($val->adultoMayor->ap_paterno ?? '')) }}
                            </td>
                            <td class="px-5 py-3">
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
            <div class="w-full max-w-xl overflow-hidden rounded-[1.5rem] bg-fondo-panel shadow-2xl">
                <div class="flex items-center justify-between border-b border-borde/70 bg-fondo-app px-6 py-4">
                    <h3 class="text-lg font-extrabold text-parrafo">Detalle de Valoracion</h3>
                ...
                <div class="p-6 space-y-4">
                    <div class="rounded-xl border border-borde/70 p-4">
                        ...
                        <p class="text-base font-extrabold text-parrafo">{{ $detalle->adultoMayor->nombre_completo ?? trim(($detalle->adultoMayor->nombres ?? 'S/D') . ' ' . ($detalle->adultoMayor->ap_paterno ?? '')) }}</p>
                        <p class="text-xs font-bold mt-1">Registrado por: {{ $detalle->registradoPor->name ?? $detalle->registradoPor->nombres ?? 'Desconocido' }}</p>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <p class="text-[10px] font-bold text-parrafo/60 uppercase">Riesgo Caida</p>
                    ...
                    @if($detalle->observacion)
                        <div>
                            <p class="text-[10px] font-bold text-parrafo/60 uppercase">Observacion</p>
                            <p class="text-sm text-parrafo">{{ $detalle->observacion }}</p>
                        </div>
                    @endif
                    @if($detalle->recomendacion_enfermeria)
                        <div>
                            <p class="text-[10px] font-bold text-parrafo/60 uppercase">Recomendacion</p>
                            <p class="text-sm text-parrafo">{{ $detalle->recomendacion_enfermeria }}</p>
                        </div>
                    @endif
                </div>
