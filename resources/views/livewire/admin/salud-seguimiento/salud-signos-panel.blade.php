...
<div class="space-y-4">
    ...
    <section class="overflow-hidden rounded-[1.6rem] border border-borde/65 bg-fondo-panel shadow-sm backdrop-blur-xl">
        ...
        <div class="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-xl font-black tracking-tight text-parrafo">Signos Vitales</h2>
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.salud-seguimiento.reportes') }}" class="inline-flex items-center justify-center gap-2 rounded-xl border border-borde bg-fondo-panel px-4 py-2 text-xs font-black uppercase tracking-wider text-parrafo transition hover:bg-fondo-app active:scale-95">
                    <i class="ph-bold ph-file-chart text-sm"></i>
                    Generar Reporte
                </a>
                @can('salud.signos.crear')
                    <button type="button" wire:click="abrirFormularioNuevo" class="inline-flex items-center justify-center gap-2 rounded-xl bg-boton-acento px-5 py-2.5 text-xs font-black uppercase tracking-wider text-inverso shadow-[0_10px_22px_rgba(226,125,96,0.24)] transition hover:-translate-y-0.5 hover:bg-fondo-panel active:scale-95">
    ...
    <section class="rounded-[1.6rem] border border-borde/65 bg-fondo-panel p-5 shadow-sm backdrop-blur-xl">
        <label class="mb-2 block text-[10px] font-black uppercase tracking-widest text-apoyo">Seleccionar Paciente</label>
        <div class="grid gap-3 md:grid-cols-[1fr_1fr_auto]">
            ...
            <div>
                <button wire:click="buscarPacienteAction" type="button" class="inline-flex h-full w-full items-center justify-center gap-2 rounded-xl bg-boton-principal px-6 text-xs font-black uppercase tracking-wider text-inverso shadow-sm transition hover:bg-fondo-panel active:scale-95">
    ...
    @if($adulto)
        ...
        <section class="rounded-[1.6rem] border border-estado-exitoBorde bg-estado-exitoBg p-4 shadow-sm backdrop-blur-xl relative overflow-hidden">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h3 class="text-base font-black text-parrafo">{{ $pacienteResumen['nombre'] }}</h3>
                        <div class="flex items-center gap-2 mt-1">
                            <span class="inline-block rounded bg-fondo-panel px-2 py-0.5 text-[9px] font-black uppercase text-apoyo">{{ $pacienteResumen['edad'] }}</span>
                            @if($pacienteResumen['estado'])
                            <span class="inline-block rounded bg-estado-exitoBg px-2 py-0.5 text-[9px] font-black uppercase text-estado-exito">{{ $pacienteResumen['estado'] }}</span>
                            @endif
                        </div>
                    </div>
                </div>
                <div class="text-right">
                    <p class="text-[10px] font-black uppercase tracking-widest text-parrafo/55">Registros Totales</p>
                    <p class="text-xl font-black text-estado-exito">{{ $pacienteResumen['vigentes'] }}</p>
        ...
        <section class="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
            @forelse($metricas as $metrica)
                <div class="relative overflow-hidden rounded-[1.4rem] border {{ $estadoBadge[$metrica['status_key']] ?? 'bg-fondo-card' }} p-3 shadow-sm backdrop-blur-md">
                    ...
                    <p class="pr-7 text-[10px] font-black uppercase tracking-widest text-apoyo">{{ $metrica['label'] }}</p>
        ...
        <section class="grid gap-4 lg:grid-cols-[1.8fr_1fr]">
            ...
            <div class="rounded-[1.6rem] border border-borde/65 bg-fondo-card p-4 shadow-sm"
                ...
                <div class="mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <h3 class="text-sm font-black uppercase tracking-wider text-parrafo">Evolución Clínica</h3>
            ...
            <div class="rounded-[1.6rem] border border-borde bg-fondo-panel p-4 shadow-sm flex flex-col justify-between">
                <div>
                    <h3 class="text-sm font-black uppercase tracking-wider text-estado-info mb-4 flex items-center gap-2">
        ...
        <section class="rounded-[1.6rem] border border-borde/65 bg-fondo-card p-5 shadow-sm backdrop-blur-xl">
            <h3 class="mb-4 text-sm font-black uppercase tracking-wider text-parrafo flex items-center gap-2">
        ...
        <section class="rounded-[1.6rem] border border-borde/65 bg-fondo-card shadow-sm overflow-hidden relative">
            <div class="border-b border-borde-suave bg-fondo-panel px-4 py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <h3 class="text-sm font-black uppercase tracking-wider text-parrafo flex items-center gap-2">
                        ...
                        @forelse($historial as $signo)
                            ...
                            <tr class="transition-colors hover:bg-fondo-panel {{ $estadoClases }}">
                                <td class="px-4 py-2.5">
                                    <p class="text-xs font-black text-parrafo">{{ $signo->fecha->format('d/m/Y') }}</p>
    ...
    @else
        <section class="rounded-[1.6rem] border border-dashed border-borde/65 bg-fondo-panel p-12 text-center shadow-inner">
            ...
            <h3 class="text-base font-black text-parrafo">Ningún paciente seleccionado</h3>
    ...
    <x-ui.modal-livewire wire:model="modalFormulario" title="{{ $signoId ? 'Editar Signos Vitales' : 'Registrar Signos Vitales' }}" maxWidth="2xl" closeMethod="cerrarModal">
        ...
        <form wire:submit.prevent="guardar" id="formSignos" x-data="{ paso: 1 }">
            ...
            <div class="mb-6 flex items-center justify-between border-b border-borde-suave pb-4">
                <template x-for="i in 5" :key="i">
                    <div class="flex-1 text-center">
                        <button type="button" @click="paso = i" class="text-[10px] font-black uppercase tracking-wider transition-colors" :class="paso >= i ? 'text-boton-acento' : 'text-meta'">
            ...
            <div x-show="paso === 1" x-transition.opacity.duration.300ms>
                <div class="rounded-[1.4rem] border border-borde/65 bg-fondo-panel p-5 shadow-sm backdrop-blur-xl">
                    <h4 class="mb-4 text-sm font-black uppercase tracking-wider text-parrafo">1. Paciente, Fecha y Hora</h4>
                    <div class="grid gap-4 md:grid-cols-3">
                        <div class="md:col-span-3">
                            <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-parrafo/55">Paciente *</label>
                            @if($adulto)
                                <div class="rounded-xl border border-borde/70 bg-fondo-card px-3 py-2.5 text-xs font-black text-parrafo">{{ $pacienteResumen['nombre'] ?? 'Adulto seleccionado' }}</div>
                            @else
                        ...
                        <div>
                            <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-parrafo/55">Fecha *</label>
                        ...
                        <div>
                            <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-parrafo/55">Hora *</label>
                ...
                <div class="mt-5 flex justify-end">
                    <button type="button" @click="paso = 2" class="rounded-xl bg-boton-principal px-5 py-2.5 text-xs font-black uppercase tracking-wider text-inverso transition hover:bg-fondo-panel">Siguiente Paso <i class="ph-bold ph-arrow-right ml-1"></i></button>
                </div>
            </div>
            ...
            <div x-show="paso === 2" x-transition.opacity.duration.300ms style="display: none;">
                <div class="rounded-[1.4rem] border border-borde/65 bg-fondo-panel p-5 shadow-sm backdrop-blur-xl">
                    <h4 class="mb-4 text-sm font-black uppercase tracking-wider text-parrafo">2. Presión Arterial</h4>
                    <div class="grid gap-4 md:grid-cols-2">
                        <div>
                            <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-parrafo/55">Sistólica (mmHg)</label>
                        ...
                        <div>
                            <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-parrafo/55">Diastólica (mmHg)</label>
                ...
                <div class="mt-5 flex justify-between">
                    <button type="button" @click="paso = 1" class="rounded-xl border border-borde/70 bg-fondo-card px-5 py-2.5 text-xs font-black uppercase tracking-wider text-parrafo"><i class="ph-bold ph-arrow-left mr-1"></i> Anterior</button>
                    <button type="button" @click="paso = 3" class="rounded-xl bg-boton-principal px-5 py-2.5 text-xs font-black uppercase tracking-wider text-inverso transition hover:bg-fondo-panel">Siguiente Paso <i class="ph-bold ph-arrow-right ml-1"></i></button>
                </div>
            </div>
            ...
            <div x-show="paso === 3" x-transition.opacity.duration.300ms style="display: none;">
                <div class="rounded-[1.4rem] border border-borde/65 bg-fondo-panel p-5 shadow-sm backdrop-blur-xl">
                    <h4 class="mb-4 text-sm font-black uppercase tracking-wider text-parrafo">3. Frecuencias y Temperatura</h4>
                    <div class="grid gap-4 md:grid-cols-2">
                        <div>
                            <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-parrafo/55">Frecuencia Cardíaca (lpm)</label>
                        ...
                        <div>
                            <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-parrafo/55">Saturación O2 (%)</label>
                        ...
                        <div>
                            <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-parrafo/55">Temperatura (°C)</label>
                        ...
                        <div>
                            <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-parrafo/55">Frecuencia Resp. (rpm)</label>
                ...
                <div class="mt-5 flex justify-between">
                    <button type="button" @click="paso = 2" class="rounded-xl border border-borde/70 bg-fondo-card px-5 py-2.5 text-xs font-black uppercase tracking-wider text-parrafo"><i class="ph-bold ph-arrow-left mr-1"></i> Anterior</button>
                    <button type="button" @click="paso = 4" class="rounded-xl bg-boton-principal px-5 py-2.5 text-xs font-black uppercase tracking-wider text-inverso transition hover:bg-fondo-panel">Siguiente Paso <i class="ph-bold ph-arrow-right ml-1"></i></button>
                </div>
            </div>
            ...
            <div x-show="paso === 4" x-transition.opacity.duration.300ms style="display: none;">
                <div class="rounded-[1.4rem] border border-borde/65 bg-fondo-panel p-5 shadow-sm backdrop-blur-xl">
                    <h4 class="mb-4 text-sm font-black uppercase tracking-wider text-parrafo">4. Peso y Talla</h4>
                    <div class="grid gap-4 md:grid-cols-3">
                        <div>
                            <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-parrafo/55">Peso (kg)</label>
                        ...
                        <div>
                            <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-parrafo/55">Talla (m o cm)</label>
                        ...
                        <div>
                            <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-parrafo/55">IMC Calculado</label>
                            <input type="text" wire:model="imc" readonly class="w-full rounded-xl border border-borde/35 bg-fondo-panel px-3 py-2.5 text-xs font-black text-parrafo outline-none">
                        </div>
                    </div>
                </div>
                <div class="mt-5 flex justify-between">
                    <button type="button" @click="paso = 3" class="rounded-xl border border-borde/70 bg-fondo-card px-5 py-2.5 text-xs font-black uppercase tracking-wider text-parrafo"><i class="ph-bold ph-arrow-left mr-1"></i> Anterior</button>
                    <button type="button" @click="paso = 5" class="rounded-xl bg-boton-principal px-5 py-2.5 text-xs font-black uppercase tracking-wider text-inverso transition hover:bg-fondo-panel">Siguiente Paso <i class="ph-bold ph-arrow-right ml-1"></i></button>
                </div>
            </div>
            ...
            <div x-show="paso === 5" x-transition.opacity.duration.300ms style="display: none;">
                <div class="rounded-[1.4rem] border border-borde/65 bg-fondo-panel p-5 shadow-sm backdrop-blur-xl">
                    <h4 class="mb-4 text-sm font-black uppercase tracking-wider text-parrafo">5. Observaciones Finales</h4>
                    ...
                    <div class="grid gap-4">
                        <div>
                            <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-parrafo/55">Observación / Nota (Opcional)</label>
                ...
                <div class="mt-5 flex justify-between">
                    <button type="button" @click="paso = 4" class="rounded-xl border border-borde/70 bg-fondo-card px-5 py-2.5 text-xs font-black uppercase tracking-wider text-parrafo"><i class="ph-bold ph-arrow-left mr-1"></i> Anterior</button>
        ...
        <x-slot name="footer">
            <button type="button" wire:click="cerrarModal" class="rounded-xl border border-borde/70 bg-fondo-card px-5 py-2.5 text-xs font-black uppercase tracking-wider text-parrafo transition hover:bg-fondo-app active:scale-95">
                Cancelar
            </button>
            <button type="submit" form="formSignos" class="inline-flex items-center justify-center gap-2 rounded-xl bg-boton-acento px-6 py-2.5 text-xs font-black uppercase tracking-wider text-inverso shadow-[0_8px_16px_rgba(226,125,96,0.2)] transition hover:-translate-y-0.5 hover:bg-fondo-panel active:scale-95 disabled:opacity-50" wire:loading.attr="disabled" wire:target="guardar,confirmarGuardarConAlertas">
    ...
    <x-ui.modal-livewire wire:model="modalPresion" title="Registrar Presión Arterial" maxWidth="md" closeMethod="cerrarModal">
        ...
        <form wire:submit.prevent="guardar" id="formPresion">
            <div class="grid gap-4">
                ...
                <div>
                    <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-parrafo/55">Observación (Opcional)</label>
        ...
        <x-slot name="footer">
            <button type="button" wire:click="cerrarModal" class="rounded-xl border border-borde/70 bg-fondo-card px-5 py-2.5 text-xs font-black uppercase tracking-wider text-parrafo transition hover:bg-fondo-app active:scale-95">Cancelar</button>
            <button type="submit" form="formPresion" class="inline-flex items-center gap-2 rounded-xl bg-boton-acento px-6 py-2.5 text-xs font-black uppercase tracking-wider text-inverso shadow-sm transition hover:bg-fondo-panel active:scale-95">Guardar</button>
        </x-slot>
    </x-ui.modal-livewire>
    ...
    <x-ui.modal-livewire wire:model="modalCardiaca" title="FC y Saturación" maxWidth="md" closeMethod="cerrarModal">
        ...
        <form wire:submit.prevent="guardar" id="formCardiaca">
            <div class="grid gap-4">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-parrafo/55">Frecuencia (lpm)</label>
                    ...
                    <div>
                        <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-parrafo/55">Saturación (%)</label>
        ...
        <x-slot name="footer">
            <button type="button" wire:click="cerrarModal" class="rounded-xl border border-borde/70 bg-fondo-card px-5 py-2.5 text-xs font-black uppercase tracking-wider text-parrafo transition hover:bg-fondo-app active:scale-95">Cancelar</button>
            <button type="submit" form="formCardiaca" class="inline-flex items-center gap-2 rounded-xl bg-boton-acento px-6 py-2.5 text-xs font-black uppercase tracking-wider text-inverso shadow-sm transition hover:bg-fondo-panel active:scale-95">Guardar</button>
        </x-slot>
    </x-ui.modal-livewire>
    ...
    <x-ui.modal-livewire wire:model="modalTemperatura" title="Temp y Respiración" maxWidth="md" closeMethod="cerrarModal">
        ...
        <form wire:submit.prevent="guardar" id="formTemperatura">
            <div class="grid gap-4">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-parrafo/55">Resp. (rpm)</label>
        ...
        <x-slot name="footer">
            <button type="button" wire:click="cerrarModal" class="rounded-xl border border-borde/70 bg-fondo-card px-5 py-2.5 text-xs font-black uppercase tracking-wider text-parrafo transition hover:bg-fondo-app active:scale-95">Cancelar</button>
            <button type="submit" form="formTemperatura" class="inline-flex items-center gap-2 rounded-xl bg-boton-acento px-6 py-2.5 text-xs font-black uppercase tracking-wider text-inverso shadow-sm transition hover:bg-fondo-panel active:scale-95">Guardar</button>
        </x-slot>
    </x-ui.modal-livewire>
    ...
    <x-ui.modal-livewire wire:model="modalPeso" title="Peso e IMC" maxWidth="md" closeMethod="cerrarModal">
        ...
        <form wire:submit.prevent="guardar" id="formPeso">
            <div class="grid gap-4">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-parrafo/55">Talla (m)</label>
        ...
        <x-slot name="footer">
            <button type="button" wire:click="cerrarModal" class="rounded-xl border border-borde/70 bg-fondo-card px-5 py-2.5 text-xs font-black uppercase tracking-wider text-parrafo transition hover:bg-fondo-app active:scale-95">Cancelar</button>
            <button type="submit" form="formPeso" class="inline-flex items-center gap-2 rounded-xl bg-boton-acento px-6 py-2.5 text-xs font-black uppercase tracking-wider text-inverso shadow-sm transition hover:bg-fondo-panel active:scale-95">Guardar</button>
        </x-slot>
    </x-ui.modal-livewire>
    ...
    <x-ui.modal-livewire wire:model="modalObservacion" title="Registrar Observación" maxWidth="md" closeMethod="cerrarModal">
        ...
        <form wire:submit.prevent="guardar" id="formObservacion">
            <div class="grid gap-4">
                ...
                <div>
                    <label class="mb-1 block text-[11px] font-black uppercase tracking-widest text-parrafo/55">Observación Clínica *</label>
        ...
        <x-slot name="footer">
            <button type="button" wire:click="cerrarModal" class="rounded-xl border border-borde/70 bg-fondo-card px-5 py-2.5 text-xs font-black uppercase tracking-wider text-parrafo transition hover:bg-fondo-app active:scale-95">Cancelar</button>
            <button type="submit" form="formObservacion" class="inline-flex items-center gap-2 rounded-xl bg-boton-acento px-6 py-2.5 text-xs font-black uppercase tracking-wider text-inverso shadow-sm transition hover:bg-fondo-panel active:scale-95">Guardar</button>
        </x-slot>
    </x-ui.modal-livewire>
</div>
