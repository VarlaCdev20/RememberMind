<div class="space-y-6">
    <section class="overflow-hidden rounded-[1.6rem] border border-borde/65 bg-fondo-panel shadow-sm backdrop-blur-xl">
        ...
        <div class="grid gap-4 p-5 lg:grid-cols-[1fr_auto] lg:items-end">
            <div>
                <span class="text-[10px] font-black uppercase tracking-[0.2em] text-boton-acento">Monitoreo preventivo</span>
                <h2 class="mt-1 text-xl font-black tracking-tight text-parrafo">Alertas de salud</h2>
            ...
            <div class="grid grid-cols-3 gap-2 sm:min-w-[360px]">
                <div class="rounded-2xl border border-borde/45 bg-fondo-panel p-3 text-center">
                    ...
                    <p class="mt-1 text-xl font-black text-parrafo">{{ $conteos['total'] ?? count($alertas) }}</p>
                </div>
                <div class="rounded-2xl border border-borde-focus bg-estado-peligroBg p-3 text-center">
                    ...
                    <p class="mt-1 text-xl font-black text-boton-acento">{{ $conteos['criticas'] ?? 0 }}</p>
                </div>
                <div class="rounded-2xl border border-estado-advertenciaBorde bg-estado-advertenciaBg p-3 text-center">
                    ...
                    <p class="mt-1 text-xl font-black text-parrafo">{{ $conteos['preventivas'] ?? 0 }}</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="rounded-[1.6rem] border border-borde/65 bg-fondo-panel p-4 shadow-sm backdrop-blur-xl sm:p-5">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
                <h3 class="text-sm font-black uppercase tracking-wider text-parrafo">Filtro clínico</h3>
            ...
            <label class="block w-full sm:max-w-xs">
                <span class="mb-1.5 block text-[9px] font-black uppercase tracking-widest text-parrafo/55">Tipo de alerta</span>
                <div class="relative">
    ...
    <section class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            ...
            <article class="group overflow-hidden rounded-[1.55rem] border {{ $panel['border'] }} {{ $panel['bg'] }} shadow-sm backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_38px_rgba(47,62,92,0.14)]">
                <div class="flex items-start gap-4 border-b border-borde-suave bg-fondo-panel p-5">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <h3 class="mt-2 truncate text-base font-black text-parrafo">
                            {{ $alerta['adulto']->nombres }} {{ $alerta['adulto']->ap_paterno }}
                        </h3>
                        <p class="mt-0.5 text-[10px] font-black uppercase tracking-wider text-parrafo/42">{{ $alerta['adulto']->cod_am }}</p>
                    </div>
                </div>
                ...
                <div class="space-y-4 p-5">
                    ...
                    <div class="rounded-2xl border border-borde/35 bg-fondo-panel px-4 py-3">
                        <p class="text-[9px] font-black uppercase tracking-widest text-parrafo/45">Acción sugerida</p>
                        <p class="mt-1 text-xs font-black uppercase text-parrafo">{{ $alerta['accion'] }}</p>
                    </div>
                    <a href="{{ $alerta['ruta'] }}" class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-boton-principal px-4 py-2.5 text-[10px] font-black uppercase tracking-wider text-inverso shadow-[0_8px_18px_rgba(47,62,92,0.18)] transition hover:bg-fondo-panel active:scale-95">
        ...
        @empty
            <div class="col-span-full rounded-[1.6rem] border border-estado-exitoBorde bg-estado-exitoBg p-12 text-center shadow-sm">
                ...
                <h3 class="mt-4 text-lg font-black text-parrafo">Sin alertas activas</h3>
