<div class="space-y-6">
    <section class="overflow-hidden rounded-[1.6rem] border border-borde/65 bg-fondo-panel shadow-sm backdrop-blur-xl">
        ...
        <div class="p-5">
            <span class="text-[10px] font-black uppercase tracking-[0.2em] text-boton-acento">Documentación institucional</span>
            <h2 class="mt-1 text-xl font-black tracking-tight text-parrafo">Reportes de salud</h2>
    ...
    <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        @foreach([
            ...
            <button type="button" wire:click="$set('tipoReporte', '{{ $card['key'] }}')" class="group rounded-[1.45rem] border p-5 text-left shadow-sm backdrop-blur-xl transition hover:-translate-y-1 hover:shadow-[0_16px_34px_rgba(47,62,92,0.12)] {{ $tipoReporte === $card['key'] ? 'border-borde-focus bg-fondo-panel' : 'border-borde bg-fondo-panel' }}">
                ...
                <h3 class="mt-4 text-base font-black text-parrafo">{{ $card['title'] }}</h3>
    ...
    <section class="overflow-hidden rounded-[1.6rem] border border-borde/65 bg-fondo-panel shadow-sm backdrop-blur-xl">
        <div class="grid lg:grid-cols-[240px_1fr]">
            <aside class="border-b border-borde/45 bg-fondo-panel p-6 lg:border-b-0 lg:border-r">
                ...
                <h3 class="mt-4 text-lg font-black text-parrafo">Generador</h3>
                ...
                <div class="mt-5 rounded-2xl border border-borde-suave bg-fondo-panel p-3">
                    <p class="text-[9px] font-black uppercase tracking-widest text-parrafo/45">Estado</p>
                    <p class="mt-1 text-xs font-black text-estado-exito">Disponible para vista previa</p>
                </div>
            </aside>
            ...
            <div class="p-5 sm:p-7">
                <form wire:submit.prevent="generarVistaPrevia" class="space-y-5">
                    <div class="grid gap-4 md:grid-cols-2">
                        <div>
                            <label class="mb-1.5 block text-[10px] font-black uppercase tracking-widest text-apoyo">Adulto mayor</label>
                            ...
                            @error('adultoSeleccionado') <span class="mt-1 flex items-center text-[10px] font-black text-boton-acento"><i class="ph-bold ph-warning mr-1"></i>{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="mb-1.5 block text-[10px] font-black uppercase tracking-widest text-apoyo">Tipo de reporte</label>
                            ...
                            @error('tipoReporte') <span class="mt-1 flex items-center text-[10px] font-black text-boton-acento"><i class="ph-bold ph-warning mr-1"></i>{{ $message }}</span> @enderror
                        </div>
                    </div>

                    <div class="grid gap-4 border-t border-borde/35 pt-5 md:grid-cols-2">
                        <div>
                            <label class="mb-1.5 block text-[10px] font-black uppercase tracking-widest text-apoyo">Fecha inicio</label>
                        ...
                        <div>
                            <label class="mb-1.5 block text-[10px] font-black uppercase tracking-widest text-apoyo">Fecha fin</label>
                    ...
                    <div class="flex flex-col-reverse gap-2 border-t border-borde/35 pt-5 sm:flex-row sm:justify-end">
                        <button type="submit" class="inline-flex items-center justify-center gap-2 rounded-xl border border-borde/70 bg-fondo-panel px-5 py-2.5 text-xs font-black uppercase tracking-wider text-parrafo transition hover:bg-fondo-panel active:scale-95">
                            <i class="ph-bold ph-eye text-sm"></i>
                            Vista previa
                        </button>
                        <button type="button" onclick="window.print()" class="inline-flex items-center justify-center gap-2 rounded-xl bg-boton-principal px-5 py-2.5 text-xs font-black uppercase tracking-wider text-inverso shadow-[0_8px_18px_rgba(47,62,92,0.18)] transition hover:-translate-y-0.5 hover:bg-fondo-panel active:scale-95">
    ...
    <section class="hidden print:block rounded-2xl border border-borde-suave bg-fondo-card p-8 shadow-sm">
        <div class="mb-8 border-b border-borde-suave pb-5 text-center">
            ...
            <p class="mt-1 text-[10px] font-black uppercase tracking-widest text-boton-acento">Reporte institucional de salud</p>
        </div>
        <div class="flex flex-col items-center justify-center rounded-xl border border-dashed border-borde bg-fondo-panel py-12 text-center">
            ...
            <h3 class="text-base font-black text-parrafo">Documento en espera</h3>
