    ...
    <section class="overflow-hidden rounded-[1.6rem] border border-borde/65 bg-fondo-panel shadow-sm backdrop-blur-xl">
        ...
        <div class="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <span class="text-[10px] font-black uppercase tracking-[0.2em] text-boton-acento">Control diario de tomas</span>
                    <h2 class="text-xl font-black tracking-tight text-parrafo">Administración de medicación</h2>
            ...
            @can('salud.medicacion.crear')
                <button type="button" @click="$dispatch('abrirModalMedicacion', { cod_am: '{{ $adulto->cod_am }}' })" class="inline-flex items-center justify-center gap-2 rounded-xl border border-borde/70 bg-fondo-panel px-5 py-2.5 text-xs font-black uppercase tracking-wider text-parrafo shadow-sm transition hover:bg-fondo-panel active:scale-95">
    ...
    <section class="grid grid-cols-2 gap-3 sm:grid-cols-4">
        @foreach([
            ...
            <div class="relative overflow-hidden rounded-2xl border {{ $item['border'] }} {{ $item['bg'] }} p-4 shadow-sm backdrop-blur-md">
                ...
                <p class="pr-7 text-[9px] font-black uppercase tracking-[0.15em] text-parrafo/48">{{ $item['label'] }}</p>
    ...
    <section class="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
        <div class="rounded-[1.6rem] border border-borde/65 bg-fondo-panel p-4 shadow-sm backdrop-blur-xl sm:p-5">
            <div class="mb-4 flex items-center justify-between gap-3">
                <div>
                    <h3 class="text-sm font-black uppercase tracking-wider text-parrafo">Tomas programadas</h3>
            ...
            <div class="space-y-3">
                @forelse($medicacionesActivas as $med)
                    <article class="rounded-[1.35rem] border border-borde/45 bg-fondo-panel p-4 shadow-sm">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                <h4 class="truncate text-sm font-black text-parrafo">{{ $med->nombre_medicamento }}</h4>
                                <p class="mt-1 text-[11px] font-bold text-apoyo">{{ $med->dosis }} - {{ $med->frecuencia }} - {{ $med->via_administracion }}</p>
                                <p class="mt-1 text-[10px] font-black uppercase tracking-wider text-boton-acento">
                                    <i class="ph-bold ph-clock mr-1"></i>
                                    {{ $med->hora_programada ? \Carbon\Carbon::parse($med->hora_programada)->format('H:i') : 'Sin hora' }}
                                </p>
                            </div>
                            <span class="rounded-full border border-estado-exitoBorde bg-estado-exitoBg px-2.5 py-1 text-[9px] font-black uppercase text-estado-exito">Activa</span>
                        </div>

                        @can('salud.administracion.crear')
                            <button type="button" @click="$dispatch('abrirModalAdministracion', { cod_am: '{{ $adulto->cod_am }}', cod_med_adulto: {{ $med->cod_med_adulto }} })" class="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-boton-principal px-4 py-2.5 text-[10px] font-black uppercase tracking-wider text-inverso shadow-[0_8px_18px_rgba(47,62,92,0.16)] transition hover:bg-fondo-panel active:scale-95">
                ...
                @empty
                    <div class="rounded-[1.4rem] border border-dashed border-borde/70 bg-fondo-panel p-8 text-center shadow-inner">
                        ...
                        <h3 class="mt-3 text-base font-black text-parrafo">Sin medicaciones activas</h3>
        ...
        <div class="rounded-[1.6rem] border border-borde/65 bg-fondo-panel p-4 shadow-sm backdrop-blur-xl sm:p-5">
            <div class="mb-4 flex items-center justify-between gap-3">
                <div>
                    <h3 class="text-sm font-black uppercase tracking-wider text-parrafo">Historial de administraciones</h3>
                    <p class="mt-1 text-xs font-bold text-parrafo/55">Secuencia reciente de tomas, omisiones y responsables.</p>
                </div>
                <span class="rounded-full bg-fondo-panel px-3 py-1 text-[10px] font-black uppercase tracking-wider text-parrafo/65">{{ $administraciones->count() }} registros</span>
            </div>
            ...
            <div class="space-y-3">
                        ...
                        <div class="relative flex gap-4">
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="flex flex-wrap items-center justify-between gap-2">
                                    <h4 class="truncate text-sm font-black text-parrafo">{{ $admin->medicacion?->nombre_medicamento ?? 'Medicación no disponible' }}</h4>
                                    <span class="rounded-full px-2.5 py-0.5 text-[9px] font-black uppercase tracking-wider {{ $administrado ? 'bg-estado-exitoBg text-estado-exito' : 'bg-estado-peligroBg text-boton-acento' }}">{{ $estadoTexto }}</span>
                                </div>
                ...
                @empty
                    <div class="rounded-[1.4rem] border border-dashed border-borde/70 bg-fondo-panel p-10 text-center shadow-inner">
                        ...
                        <h3 class="mt-3 text-base font-black text-parrafo">Sin administraciones registradas</h3>
