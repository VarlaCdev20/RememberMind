<div class="relative w-full" x-data="{ isOpen: @entangle('isOpen') }" @click.outside="isOpen = false" @keydown.escape.window="isOpen = false">
    ...
    <div class="relative flex items-center">
        ...
        <input 
            ...
            class="rm-input rounded-full pl-9 pr-9 w-full bg-fondo-card/50 text-sm shadow-sm"
    ...
    <div 
        ...
        class="absolute top-full left-0 right-0 mt-2 w-full min-w-[300px] z-50 rounded-xl dropdown-institucional shadow-lg max-h-[70vh] overflow-y-auto"
        x-cloak
    >
                        ...
                        <div>
                            ...
                            <ul class="space-y-0.5">
                                @foreach($grupo['items'] as $item)
                                    <li>
                                        <a href="{{ $item['url'] }}" class="flex items-start gap-3 px-2 py-2 rounded-lg transition bg-transparent hover:bg-fondo-app hover:shadow-sm active:scale-95 group border border-transparent hover:border-borde-suave">
                                            ...
                                            <div class="min-w-0 flex-1">
                                                <div class="flex items-center justify-between gap-2">
                                                    <p class="text-sm font-bold text-titulo truncate">{{ $item['titulo'] }}</p>
                                                ...
                                                <p class="text-[10px] font-bold text-parrafo truncate mt-0.5">{{ $item['subtitulo'] }}</p>
                                                @if(isset($item['detalle']))
                                                    <p class="text-[9px] font-semibold text-meta truncate mt-0.5 group-hover:text-parrafo transition">{{ $item['detalle'] }}</p>
                                                @endif
                                            </div>
