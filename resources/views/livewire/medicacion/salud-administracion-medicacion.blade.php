<div class="space-y-6" x-data>
 ...
 @php
 $med = $toma['medicacion'];
 $sinRegistro = !$toma['registro'];
 $estadoClase = match($toma['estado']) {
 ...
 <span class="rounded-full border px-2.5 py-1 text-[9px] font-bold uppercase {{ $estadoClase }}">{{ $toma['estado'] === 'PROXIMA' ? 'Próxima' : ucfirst(strtolower($toma['estado'])) }}</span>
 </div>

 @if($sinRegistro && auth()->user()?->can('administracion_medicacion.registrar'))
 <button type="button" @click="$dispatch('abrirModalAdministracion', { cod_am: '{{ $adulto->cod_am }}', cod_med_adulto: '{{ $med->cod_med_adulto }}', hora_programada: '{{ $toma['hora'] }}' })" class="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-boton-principal px-4 py-2.5 text-[10px] font-bold uppercase tracking-wider text-inverso shadow-[0_8px_18px_rgba(47,62,92,0.16)] transition hover:bg-boton-principalHover active:scale-95">
 <i class="ph-bold ph-check-square"></i>
 Registrar toma u omisión
 </button>
 @elseif($toma['registro'])
 <p class="mt-4 text-center text-[10px] font-bold text-apoyo">La dosis de hoy ya fue registrada.</p>
 @endif
 </article>
 @empty
