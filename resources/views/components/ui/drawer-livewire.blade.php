...
<div x-data="{ show: @entangle($attributes->wire('model')) }" x-show="show" x-cloak
    ...
    <aside x-show="show" x-transition:enter="transition ease-out duration-300"
           ...
           class="rm-drawer absolute inset-y-0 right-0 flex w-full {{ $width }} flex-col border-l border-borde bg-fondo-card shadow-2xl">
        <header class="flex items-center justify-between border-b border-borde bg-fondo-hover px-5 py-4">
        ...
        @isset($footer)<footer class="border-t border-borde bg-fondo-hover p-4">{{ $footer }}</footer>@endisset
    </aside>
</div>
