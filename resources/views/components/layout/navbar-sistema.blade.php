...
>
    <div class="flex items-center justify-between gap-4">
        <div class="flex items-center gap-3">
            ...
            <div class="min-w-0">
                <p class="text-xs font-black uppercase tracking-[0.22em] text-modulo-salud">
                    RememberMind
                </p>
                <h1 class="max-w-[230px] text-[11px] font-black uppercase leading-[1.05] text-titulo sm:text-xs">
        ...
        <div class="flex items-center gap-2">
            ...
            <div x-data="{ abierto:false }" class="relative">
                ...
                >
                    <div class="flex h-7 w-7 items-center justify-center rounded-full bg-boton-principal text-xs font-black text-boton-principalTexto shadow-card">
                        {{ strtoupper(substr(auth()->user()->nombres ?? auth()->user()->name ?? 'U', 0, 1)) }}
                    </div>

                    <div class="hidden text-left leading-tight sm:block">
                        <p class="max-w-[130px] truncate text-xs font-black text-titulo">
                ...
                >
                    <a href="{{ route('profile.show') }}"
                       class="flex items-center gap-2.5 rounded-xl px-3 py-2 text-xs font-black text-titulo transition hover:bg-fondo-hover hover:text-boton-acento active:scale-95">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit"
                                class="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-xs font-black text-boton-acento transition hover:bg-boton-acento hover:text-boton-acentoTexto active:scale-95">
