...
<x-app-layout>
 ...
 <a href="{{ route('admin.adultos-mayores.reporte-individual', ['adulto_mayor' => $adulto->cod_am, 'format' => 'pdf']) }}" class="rounded-full bg-fondo-panel px-6 py-2 text-xs font-bold text-inverso shadow-lg transition hover:opacity-90 active:scale-95">
 <i class="ph-bold ph-file-pdf mr-2"></i> PDF
 </a>
 <a href="{{ route('admin.adultos-mayores.reporte-individual', ['adulto_mayor' => $adulto->cod_am, 'format' => 'word']) }}" class="rounded-full bg-fondo-panel px-6 py-2 text-xs font-bold text-inverso shadow-lg transition hover:opacity-90 active:scale-95">
 ...
 <span class="mt-2 inline-flex items-center rounded-full px-4 py-1 text-xs font-bold uppercase tracking-widest
 ...
 <span class="text-[10px] font-bold uppercase text-apoyo">C.I.</span>
 <span class="text-sm font-bold">{{ $adulto->ci }} {{ $adulto->expedicion_ci }}</span>
 </div>
 <div class="flex justify-between items-center border-b border-borde-suave pb-2">
 <span class="text-[10px] font-bold uppercase text-apoyo">Edad</span>
 <span class="text-sm font-bold">{{ $adulto->edad }} años</span>
 </div>
 <div class="flex justify-between items-center border-b border-borde-suave pb-2">
 <span class="text-[10px] font-bold uppercase text-apoyo">Género</span>
 <span class="text-sm font-bold">{{ $adulto->genero }}</span>
 </div>
 <div class="flex justify-between items-center border-b border-borde-suave pb-2">
 <span class="text-[10px] font-bold uppercase text-apoyo">Fecha Nac.</span>
 <span class="text-sm font-bold">{{ $adulto->fecha_nac?->format('d/m/Y') }}</span>
 </div>
 <div class="flex justify-between items-center border-b border-borde-suave pb-2">
 <span class="text-[10px] font-bold uppercase text-apoyo">Fecha Ingreso</span>
 <span class="text-sm font-bold">{{ $adulto->fecha_ing?->format('d/m/Y') }}</span>
 </div>
 <div class="flex justify-between items-center border-b border-borde-suave pb-2">
 <span class="text-[10px] font-bold uppercase text-apoyo">Permanencia</span>
 <span class="text-sm font-bold">{{ $adulto->permanencia }}</span>
 </div>
 </div>
 </div>
 ...
 <h4 class="mb-4 text-xs font-bold uppercase tracking-[0.2em] text-apoyo border-b border-borde pb-2">Contactos de Referencia</h4>
 <div class="space-y-4">
 ...
 <div>
 <p class="text-sm font-bold leading-tight">{{ $fam->usuario->name ?? 'Familiar' }}</p>
 ...
 <h4 class="mb-4 text-xs font-bold uppercase tracking-[0.2em] opacity-50">Resumen Operativo</h4>
 <div class="grid grid-cols-2 gap-4">
 <div class="text-center">
 <p class="text-[10px] font-bold uppercase opacity-60">Atenciones</p>
 <p class="text-2xl font-black">{{ $resumen['total_atenciones'] }}</p>
 </div>
 <div class="text-center">
 <p class="text-[10px] font-bold uppercase opacity-60">Evaluaciones</p>
 <p class="text-2xl font-black">{{ $resumen['total_evaluaciones'] }}</p>
 </div>
 </div>
 <div class="mt-4 pt-4 border-t border-white/10 text-center">
 <p class="text-[10px] font-bold uppercase opacity-60">Último Seguimiento</p>
 ...
 <h3 class="text-xl font-extrabold uppercase tracking-widest flex items-center gap-3">
 ...
 <div>
 <h4 class="text-base font-extrabold text-titulo">{{ $eval->tipoEvaluacion->nombre }} — {{ $eval->puntaje_total }} pts.</h4>
 ...
 <span class="rounded-full bg-fondo-card px-4 py-1 text-[10px] font-bold shadow-sm">{{ $eval->fecha_eval->format('d/m/Y') }}</span>
 <span class="rounded-full px-4 py-1 text-[10px] font-bold {{ $eval->nivel_riesgo === 'BAJO' ? 'bg-fondo-panel text-inverso' : 'bg-fondo-panel text-inverso' }}">RIESGO {{ $eval->nivel_riesgo }}</span>
 </div>
 </div>
 ...
 <div class="rounded-[2.5rem] border border-borde bg-fondo-card p-8 shadow-sm">
 <h3 class="text-xl font-extrabold uppercase tracking-widest flex items-center gap-3 mb-6">
 ...
 <p class="text-xs font-bold text-boton-acento">{{ $aten->fecha->format('d/m') }}</p>
 <p class="text-[10px] font-bold text-apoyo">{{ $aten->hora }}</p>
 </div>
 <div class="flex-1">
 <h4 class="text-sm font-bold uppercase tracking-wide">{{ $aten->tipoAtencion->tipo ?? 'Atención General' }}</h4>
 <p class="text-xs font-semibold text-apoyo mt-1 line-clamp-2">{{ $aten->obs ?? $aten->descripcion }}</p>
 </div>
 <div class="text-right">
 <span class="text-[9px] font-bold uppercase px-2 py-0.5 rounded bg-fondo-panel text-apoyo">{{ $aten->estado }}</span>
 </div>
 </div>
 @empty
 ...
 <p class="text-[10px] font-bold text-parrafo uppercase tracking-widest mb-1">{{ \Carbon\Carbon::parse($obs->fecha)->format('d/m/Y') }} · {{ $obs->tipo_obs }}</p>
 ...
 <h3 class="text-sm font-bold uppercase tracking-widest flex items-center gap-3 mb-6 opacity-60">
 ...
 <p class="text-xs font-bold uppercase tracking-widest text-titulo">Sello / Firma Personal de Salud</p>
 <p class="text-[10px] font-bold text-apoyo mt-1 uppercase">Validación de Perfil Cognitivo</p>
 </div>
 <div class="text-center">
 <div class="mx-auto mb-4 h-0.5 w-64 bg-fondo-panel"></div>
 <p class="text-xs font-bold uppercase tracking-widest text-titulo">Sello / Firma Dirección Administrativa</p>
 <p class="text-[10px] font-bold text-apoyo mt-1 uppercase">Autorización de Expediente</p>
 </div>
 </div>
 
 <div class="mt-20 text-center space-y-2">
 <p class="text-[10px] font-bold text-apoyo uppercase tracking-[0.3em]">REPORTE GENERADO INSTITUCIONALMENTE POR EL SISTEMA REMEMBERMIND</p>
 ...
 <h3 class="text-xl font-extrabold uppercase tracking-tighter flex items-center gap-3 mb-8">
 ...
 * El siguiente anexo contiene información que ha sido retirada de la ficha activa por motivos administrativos o médicos, pero que se conserva institucionalmente para fines de auditoría y trazabilidad histórica.
 ...
 <h4 class="text-[10px] font-bold uppercase tracking-widest text-apoyo mb-3 border-l-4 border-terracota pl-2">Observaciones Anuladas</h4>
 ...
 <div class="opacity-80 grayscale-[50%]">
 <h4 class="text-[10px] font-bold uppercase tracking-widest text-apoyo mb-3 border-l-4 border-borde pl-2">Atenciones Médicas Anuladas</h4>
 <table class="w-full text-[10px] text-left border-collapse border border-borde-suave">
 <thead class="bg-fondo-panel">
 <tr>
 <th class="p-2 border border-borde-suave">Fecha Anul.</th>
 ...
 <div class="opacity-80 grayscale-[50%]">
 <h4 class="text-[10px] font-bold uppercase tracking-widest text-apoyo mb-3 border-l-4 border-borde pl-2">Evaluaciones Cognitivas Anuladas</h4>
 <table class="w-full text-[10px] text-left border-collapse border border-borde-suave">
 <thead class="bg-fondo-panel">
 <tr>
 <th class="p-2 border border-borde-suave">Fecha Anul.</th>
