...
use App\Models\{AdultoMayor, User, TurnoEnfermeria, PlanCuidado, EjecucionCuidado, PaseTurno, Alerta, AdministracionMedicacion, Atencion, AsignacionResidenteJornada, AsignacionPersonal};
...
class CuidadosFlujoTest extends TestCase
{
    ...
    public function test_tarea_pendiente_genera_alerta_resultado_y_pase_conservan_historial(): void
    {
        ...
        foreach ([[$salienteUser, $saliente], [$entranteUser, $entrante]] as [$enfermero, $turno]) {
            AsignacionResidenteJornada::create(['cod_am' => $adulto->cod_am, 'cod_turno' => $turno->cod_turno, 'cod_usu_enfermero' => $enfermero->cod_usu, 'fecha_inicio' => today(), 'estado' => 'ACTIVA', 'nivel_supervision' => 'ESTANDAR']);
        }
        AsignacionPersonal::create(['cod_turno' => $saliente->cod_turno, 'cod_usuario' => $salienteUser->cod_usu, 'fecha_hora_recepcion' => now()]);
        ...
        $tarea = EjecucionCuidado::sole();
        $this->assertSame($plan->cod_plan, $tarea->cod_plan);
        $this->assertDatabaseHas('alertas', ['cod_am' => $adulto->cod_am, 'origen' => 'PLAN']);
        ...
        AsignacionPersonal::create(['cod_turno' => $entrante->cod_turno, 'cod_usuario' => $entranteUser->cod_usu, 'fecha_hora_recepcion' => now()]);
    ...
    public function test_omision_y_solicitud_medica_generan_alertas_con_origen_real(): void
    {
        ...
        $med = Prescripcion::create(['cod_am' => $adulto->cod_am, 'nombre_medicamento' => 'Medicación de prueba', 'dosis' => 'Según orden', 'frecuencia' => 'Diaria', 'fecha_inicio' => today()]);
        ...
        Atencion::create(['cod_am' => $adulto->cod_am, 'cod_turno' => $turno->cod_turno, 'fecha' => today(), 'requiere_medico' => true, 'observacion' => 'Solicita revisión médica por cambio observado.']);
        $this->assertDatabaseHas('alertas', ['cod_am' => $adulto->cod_am, 'origen' => 'MEDICACION']);
        $this->assertDatabaseHas('alertas', ['cod_am' => $adulto->cod_am, 'origen' => 'SOLICITUD_MEDICA']);
        $this->assertSame(2, Alerta::count());
        $this->assertSame(1, Prescripcion::count());
