...
use App\Livewire\Cuidados\{AsignacionTurnoPanel, PlanCuidadoPanel, AtencionPanel};
use App\Models\{AdultoMayor, User, Habitacion, Cama, TurnoEnfermeria, AsignacionResidenteJornada, PlanCuidado, Atencion};
...
class OcupacionCuidadosTest extends TestCase
{
    ...
    public function test_ocupacion_impide_doble_asignacion_integra_plan_seguimiento_y_conserva_historia(): void
    {
        ...
        AsignacionResidenteJornada::sole()->update(['cod_usu_enfermero' => $enfermero->cod_usu]);
        \App\Models\AsignacionPersonal::create(['cod_turno' => $turno->cod_turno, 'cod_usuario' => $enfermero->cod_usu, 'fecha_hora_recepcion' => now()]);
        $this->actingAs($enfermero);
        Livewire::test(AtencionPanel::class)->call('abrirCrear')->set('codAm', $adulto->cod_am)
        ...
        $this->actingAs($user);
        $panel->call('finalizarAsignacion', AsignacionResidenteJornada::sole()->cod_asig_turno);
        $this->assertNull($adulto->fresh()->cod_cama);
        $this->assertSame('DISPONIBLE', $cama->fresh()->estado);
        $this->assertSame('FINALIZADA', AsignacionResidenteJornada::sole()->estado);
